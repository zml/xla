...
def repo():
    ...
    include root at runtime (//xla/backends/gpu/codegen/metal_kernels:metal_include_root); our own entry
    sources under xla/backends/gpu/codegen/metal_kernels/mlx_entries/ #include them.
    Nothing here is compiled by Bazel -- MLX's kernels are Metal sources compiled
    at runtime by the thunk that uses them.

    Do not adopt upstream's -fno-fast-math: these kernels are compiled at
    runtime with our own flags, and the greedy output is the acceptance bar.
    """
    ...
    # deliberate act with a golden bench attached.
