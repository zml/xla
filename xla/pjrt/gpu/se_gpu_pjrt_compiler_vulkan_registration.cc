...
STREAM_EXECUTOR_REGISTER_MODULE_INITIALIZER(pjrt_register_vulkan_compiler, {
  PjRtRegisterDefaultCompiler(
      VulkanName(), std::make_unique<StreamExecutorGpuCompiler>(
                        VulkanId(),
                        stream_executor::vulkan::kVulkanPlatformId));
  CHECK_OK(StreamExecutorPlatformIdMapping::Global().AddMapping(
      stream_executor::vulkan::kVulkanPlatformId, VulkanId()));
});
