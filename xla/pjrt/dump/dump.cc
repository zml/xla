...
absl::StatusOr<std::string> GetDumpSubdirPath(absl::string_view dump_to_path,
  ...
  ABSL_RETURN_IF_ERROR(tsl::Env::Default()->RecursivelyCreateDir(dump_subdir));
  return dump_subdir;
}
...
absl::Status DumpCompileInputs(absl::string_view dump_to_path,
                               ...
                               int module_id) {
  ABSL_ASSIGN_OR_RETURN(std::string path, ResolveTestingDumpPath(dump_to_path));
  ...
  ABSL_RETURN_IF_ERROR(pjrt::MlirModuleToFile(module, module_file_name));
