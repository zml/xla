...
class MublasBackendTest : public HloHardwareIndependentTestBase {
 protected:
  void SetUp() override {
    ...
    auto* device = proto.mutable_gpu_device_info();
    device->set_threads_per_warp(128);
    auto* capability = device->mutable_musa_compute_capability();
    capability->set_architecture("mp_22");
    capability->set_major(2);
    capability->set_minor(2);
    capability->set_hardware_warp_size(128);
    capability->set_logical_subgroup_size(32);
    device->set_runtime_version("5.1.0");
    device->set_kernel_mode_driver_version("3.0.0");
    device->set_compile_time_toolkit_version("5.1.0");
...
TEST_F(MublasBackendTest, ReturnsDefaultZeroWorkspaceAlgorithm) {
...
TEST_F(MublasBackendTest, VersionContainsTargetAndExplicitLibraryState) {
  ...
  EXPECT_THAT(version, HasSubstr("musa_arch=mp_22"));
  EXPECT_THAT(version, HasSubstr("musa_runtime=5.1.0"));
  EXPECT_THAT(version, HasSubstr("musa_kernel_driver=3.0.0"));
  EXPECT_THAT(version, HasSubstr("musa_toolkit=5.1.0"));
  EXPECT_THAT(version,
