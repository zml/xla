...
TEST_P(FactoryTest, NativeFlyOwnsAlreadyMinimalGemm) {
  if (GetParam().names !=
      std::vector<Backend>{Backend::FLY, Backend::FLY_FISSION,
                           Backend::FLY_FUSION}) {
    GTEST_SKIP() << "Only applies to the ROCm Fly backend set.";
  ...
  constexpr char kHlo[] = R"(
HloModule fly_minimal_gemm_ownership

gemm {
  lhs = bf16[128,256]{1,0} parameter(0)
  rhs = bf16[256,768]{1,0} parameter(1)
  ROOT dot = bf16[128,768]{1,0} dot(lhs, rhs),
      lhs_contracting_dims={1}, rhs_contracting_dims={0}
}
...
ENTRY main {
  lhs = bf16[128,256]{1,0} parameter(0)
  rhs = bf16[256,768]{1,0} parameter(1)
  ROOT fusion = bf16[128,768]{1,0} fusion(lhs, rhs),
      kind=kCustom, calls=gemm,
      backend_config={"fusion_backend_config":{"kind":"__triton_gemm"}}
})";
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto fission_configs,
                          fission->GetSupportedConfigs(*fusion));
  EXPECT_FALSE(direct_configs.empty());
  EXPECT_TRUE(fission_configs.empty());
}

TEST_P(FactoryTest, FlyFissionRetainsExternalConcatenateAlternative) {
  if (GetParam().names !=
      std::vector<Backend>{Backend::FLY, Backend::FLY_FISSION,
                           Backend::FLY_FUSION}) {
    GTEST_SKIP() << "Only applies to the ROCm Fly backend set.";
  ...
  constexpr char kHlo[] = R"(
HloModule fly_external_concatenate_fission
...
ENTRY main {
  lhs0 = bf16[128,128]{1,0} parameter(0)
  lhs1 = bf16[128,128]{1,0} parameter(1)
  lhs = bf16[128,256]{1,0} concatenate(lhs0, lhs1), dimensions={1}
  rhs = bf16[256,256]{1,0} parameter(2)
