...
// NOTE(xla): FlashInfer's original #error here rejected any non-arch-specific
// SM90+ target. Our XLA build matrix compiles each TU for the whole capability
// list (including plain family targets like compute_120 alongside sm_120a); the
// cutlass blockscaled ops are arch-guarded and only emit real SASS on the
// sm_120a pass, so the family pass compiling to a stub is expected. This header
// is included but unused in the fp4 GEMM path anyway.
#if 0
