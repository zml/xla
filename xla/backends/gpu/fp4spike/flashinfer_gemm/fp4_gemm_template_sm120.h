...
#include "cutlass/util/packed_stride.hpp"  // cutlass::make_cute_packed_stride (was pulled in by cutlass_utils.cuh)
...
// When FI_FP4_NO_STREAMK is defined, alias the StreamK scheduler tag to the DP
// (StaticPersistent) scheduler so GemmKernelStreamK == GemmKernelDefault and the
// compiler emits only one SASS kernel per tile instead of two. StreamK rarely
// wins for the thin-M decode GEMV we target.
#ifdef FI_FP4_NO_STREAMK
#define FI_FP4_STREAMK_SCHEDULER cutlass::gemm::StaticPersistentScheduler
#else
#define FI_FP4_STREAMK_SCHEDULER cutlass::gemm::StreamKScheduler
#endif
...
#define INSTANTIATE_FP4_GEMM_KERNEL_LAUNCHER(T, CTA_M_, CTA_N_, CTA_K_, CGA_M_, CGA_N_, CGA_K_,                      \
  ...
  struct                                                                                                             \
    ...
    using GemmKernelStreamK =                                                                                        \
        cutlass::gemm::kernel::GemmUniversal<cute::Shape<int, int, int, int>, CollectiveMainloop,                    \
                                             CollectiveEpilogue, FI_FP4_STREAMK_SCHEDULER>;                          \
                                                                                                                     \
