...
// Rank-0 BF16 scale passes IsNoOpScale (dims only) but must still be applied.
TEST_F(ScaledDotRewriterElementSizeTest, ScalarScaleIsMultiplied) {
  ...
  EXPECT_TRUE(changed);
  // The rank-0 lhs scale broadcasts straight to the operand shape; the blocked
  // rhs scale broadcasts to the split shape and reshapes back. Both operands
  // are already BF16, so UpscaleBoth inserts no converts.
  EXPECT_THAT(
      module->entry_computation()->root_instruction(),
      GmockMatch(m::Dot(
