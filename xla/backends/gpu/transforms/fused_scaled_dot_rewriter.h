...
// Claims the kScaledDots this backend has a fused lowering for, and leaves the
// rest untouched.
//
...
// bounds apply, and what the call demands are the backend arm's business.
//
// Placement matters for tensor parallelism: this pass runs AFTER SPMD
// partitioning (in the post-SPMD optimization pipeline), so it rewrites each
// already-sharded kScaledDot into a per-shard fused call. The scaled-dot itself
// carries the SPMD sharding rule (kScaledDot partitions column/row parallel with
// an all-reduce), so the fused kernel comes for free on every shard.
//
// The CUDA arm (TryEmitCutlassScaledMatmul) claims the NVFP4 W4A4 scaled-dot on
// sm120a and lowers it to the cutlass$fp4_mm_dyn custom call. It is the first
// rung of the ladder: whatever it declines falls through to Triton and then to
// the generic dequant floor, both of which are always correct.
class FusedScaledDotRewriter : public HloModulePass {
 public:
  // `extra_filter` may veto individual instructions, mirroring
  // ScaledDotRewriter. Tests use it to force the ladder down a lower rung and
  // assert the rungs agree numerically; there is no environment override.
  explicit FusedScaledDotRewriter(se::GpuComputeCapability gpu_version = {},
                                  HloPredicate extra_filter = nullptr)
      : gpu_version_(gpu_version), extra_filter_(std::move(extra_filter)) {}
