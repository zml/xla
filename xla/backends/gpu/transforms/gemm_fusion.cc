...
HlosAndRequirements FuseTowardUsers(
  ...
  // A sub-byte result cannot be written out from here: the Triton gemm emitter
  // has no config for a packed store, so a fusion rooted at such a user does
  // not fall back -- it fails autotuning outright with "no configs could be
  // compiled" and takes the whole module with it. Leaving it outside makes it
  // its own fusion, which is where e.g. an activation quantize belongs anyway.
