...
// Rows of x one wide qmv threadgroup carries (MLX's launch formula caps it).
inline constexpr int kNvfp4QmvWideMaxVecs = 5;

inline int Nvfp4QmvWideVecsPerTg(int M) {
  if (M <= 1) return 1;
  const int cap = kNvfp4QmvWideMaxVecs;
...
inline Nvfp4DensePath SelectNvfp4DensePath(
    ...
    int arch_gen = kNvfp4DefaultArchGen) {
  const int limit = GetNvfp4QmvBatchLimit(static_cast<int>(K),
