...
// Output channels one decode-GEMV threadgroup computes. ONE definition, shared
// by every launch below: this must equal kROWS in
// service/gpu/metal_kernels/custom/fp8_gemv_pc.metal and
// MetalFp8GemvThunk::rows_per_group. When it was written out per call site, a
// stale `kN / 4` survived the change to 2 and left the tail of `out` unwritten.
...
// N odd, so the last threadgroup computes one real channel and one clamped
// onto it. Catches a dispatch whose group count disagrees with the shader's
// kROWS: at N/4 groups this case loses half its columns, silently.
...
TEST_F(MetalFp8KernelTest, PerTensorGemvKeepsAnF32ScaleExact) {
  ...
  TF_ASSERT_OK(kernel->Launch(se::ThreadDim(256, 1, 1),
                              // kROWS output channels per threadgroup; a stale
                              // divisor here leaves the tail of `out` at its
                              // NaN sentinel rather than failing loudly on its
                              // own terms.
