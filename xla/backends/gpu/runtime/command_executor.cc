...
CommandExecutor::CommandExecutor(
  ...
  // Map each buffer allocation index to the commands that reference it, so an
  // update only visits the commands whose allocations changed.
...
absl::Status CommandExecutor::RecordUpdate(
  ...
  auto skip_command_update = [&](CommandId id) {
    ...
    if (!requires_initialization_update) {
      ...
      if (command_update_marks != nullptr && !(*command_update_marks)[id]) {
        return true;
      }
    }
