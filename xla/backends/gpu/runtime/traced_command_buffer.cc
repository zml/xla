...
TracedCommandBuffer::TracedCommandBuffer(const Command* trace_cmd,
  ...
  allocs_indices_.erase(std::unique(allocs_indices_.begin(),
                                    allocs_indices_.end()),
                        allocs_indices_.end());
}
...
absl::StatusOr<se::CommandBuffer*> TracedCommandBuffer::GetOrTraceCommandBuffer(
  ...
  TF_ASSIGN_OR_RETURN(entry.command_buffer,
                      se::TraceCommandBufferFactory::Create(executor, stream,
                                                            trace));
...
bool TracedCommandBuffer::RecordedChildIsCurrent(
    const se::CommandBuffer::Command* command, se::CommandBuffer* nested) const {
