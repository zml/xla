...
absl::StatusOr<stream_executor::ThreadDim> ExtractThreadDims(
  ...
  // AMD/ROCm and Intel XPU Triton backends do not use NVVM launch attributes.
  ...
  if (target.getValue().find("gfx") != std::string::npos ||
    ...
    const int64_t warp_count =
        target.getValue().find("xpu") != std::string::npos &&
                total_num_warps_attr
            ? total_num_warps_attr.getInt()
            : num_warps_attr.getInt();
    stream_executor::ThreadDim thread_dims(
        warp_count * threads_per_warp_attr.getInt(), 1, 1);
    return thread_dims;
  }
  auto total_num_warps_attr =
      triton_module->getAttrOfType<mlir::IntegerAttr>("ttg.total-num-warps");
