...
bool CollectiveKernelEnabled(const HloInstruction& instruction,
                             DebugOptions::CollectiveKernelType type) {
...
GetBlockLevelFusionConfigForAllReduce(
  ...
  const bool enabled =
      ...
      CollectiveKernelEnabled(
          *all_reduce, DebugOptions::COLLECTIVE_KERNEL_ALL_REDUCE);
...
absl::StatusOr<AllGatherEmitterContext> CreateAllGatherEmitterContext(
  ...
  if (ctx.element_type == PrimitiveType::F8E4M3FN ||
      ctx.element_type == PrimitiveType::F8E5M2 ||
      ctx.element_type == PrimitiveType::S4) {
    return absl::InvalidArgumentError(
...
GetBlockLevelFusionConfigForAllGather(
    ...
    const DeviceAssignment* device_assignment) {
  if (!CollectiveKernelEnabled(
          *all_gather, DebugOptions::COLLECTIVE_KERNEL_ALL_GATHER)) {
    return std::nullopt;
  }
...
absl::StatusOr<std::vector<Shape>> GetAllGatherUnmanagedKernelArguments(
  ...
  if (!all_gather->device_list()) {
    return absl::InternalError("All-gather has no device list.");
  }
  const int32_t num_devices =
      all_gather->device_list()->num_devices_per_group();
  std::vector<Shape> arguments;
  arguments.reserve(computation->num_parameters() +
                    kNumCollectiveMetadataArgs);
  arguments.push_back(ShapeUtil::MakeShape(S32, {}));
  arguments.push_back(ShapeUtil::MakeShape(S32, {}));
  arguments.push_back(
  ...
  for (const HloInstruction* parameter :
    ...
    arguments.push_back(
        ShapeUtil::InsertDimensionAtIndex(parameter->shape(), 0, num_devices));
  }
  TF_RET_CHECK(arguments.size() ==
               computation->num_parameters() + kNumCollectiveMetadataArgs);
  return arguments;
}
