...
// Alignment is 16 (entry param). Do not use allocation alignment:
// the kernel gets base+slice.offset(); overstated div_by segfaulted cuLaunchKernel.
...
absl::StatusOr<llvm::SmallVector<int64_t>> PhysicalStrides(
  ...
  ASSIGN_OR_RETURN(llvm::SmallVector<int64_t> minor_to_major,
...
// Peel rhs-scale transpose only for canonical [K,N] rhs; non-canonical
// already matches mmaf_scaled and peeling would transpose it wrongly.
...
absl::StatusOr<Value> KernelConverter::GetPartitionView(
  ...
  if (!tensor_view) {
  // cuda_tile has no rank-0 view: model scalars as one element of rank-1.
    ...
    if (shape.empty()) {
      ...
      ASSIGN_OR_RETURN(strides, PhysicalStrides(memref));
    }
  ...
  // div_by is buffer size (TMA txn size), not address alignment —
  // claiming 16 for a 4-byte scalar reads past the allocation.
  ...
  // OOB tile elements need a padding value (e.g. decode 16-row tile of 1-row act)
  // or the kernel faults in the driver.
...
llvm::SmallVector<Value> KernelConverter::GetTileIndex(
  ...
  // Offsets are index*extent; peel multiply (divide fallback for tileiras).
...
Value KernelConverter::MaybeUnpack(Value tile, mlir::Type elem_type,
  ...
  // Sub-byte storage is i8 with halved packed dim; unpack only if packed dim is minor.
...
mlir::LogicalResult KernelConverter::ConvertExtract(
  ...
  // Rank-0 buffer is promoted to rank-1 in the view; load/store match that.
...
// Triton carries UE8M0 scales as i8; mmaf_scaled is typed — restore the float type.
...
mlir::LogicalResult KernelConverter::ConvertDotScaled(
  ...
  // mmaf_scaled is [M,K]x[K,N]; permute [N,K] operands rather than declining.
  if (lhs_c != 1) lhs = Transpose2D(lhs);
  if (rhs_c != 0) rhs = Transpose2D(rhs);

  Value lhs_scale = RestoreScaleType(map_.lookup(op.getLhsScale()));
  // Canonical rhs: peel scale transpose for mmaf_scaled [K/block,N].
...
mlir::LogicalResult KernelConverter::ConvertOp(mlir::Operation* op) {
  ...
  // cuda_tile.broadcast cannot change rank; reshape first.
...
bool IsLowerableToCudaTile(mlir::Operation* module) {
  ...
  // mmaf_scaled needs same low-precision operands; bf16xfp4 has no spelling here.
