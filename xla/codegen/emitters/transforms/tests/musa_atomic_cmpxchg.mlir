...
func.func @atomic_replace_bf16(%input: tensor<8xbf16>, %index: index, %update: bf16)
    -> tensor<8xbf16> {
  %result = xla.atomic_rmw %input[%index] : tensor<8xbf16> {
    ^bb0(%current : bf16):
      xla.yield %update : bf16
  }
  return %result : tensor<8xbf16>
}

// CHECK-LABEL: func.func @atomic_replace_bf16
// CHECK: llvm.ptrtoint
// CHECK: llvm.and
// CHECK: llvm.getelementptr
// CHECK: llvm.addrspacecast
// CHECK: scf.while
...
// CHECK-DAG: llvm.and
// CHECK: llvm.or
// CHECK: llvm.cmpxchg {{.*}} monotonic monotonic {alignment = 4 : i64} : !llvm.ptr<1>, i32
// CHECK-NOT: llvm.atomicrmw
