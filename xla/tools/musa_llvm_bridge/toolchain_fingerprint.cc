...
absl::StatusOr<std::string> MusaCompilerFileSha256(absl::string_view path) {
  absl::StatusOr<HashedFile> hashed =
      HashFile(path, kMaxToolBytes, /*retain_contents=*/false);
  if (!hashed.ok()) return hashed.status();
  return std::move(hashed->sha256);
}
...
absl::StatusOr<MusaBridgeFingerprints> FingerprintMusaBridgeToolchain(
  ...
  const std::pair<absl::string_view, absl::string_view> component_paths[] = {
      ...
      {"llvm_pass_plugin", paths.llvm_pass_plugin},
  };
  ...
  for (const auto& [name, path] : component_paths) {
    if (name == "llvm_pass_plugin" && path.empty()) continue;
  ...
  result.toolchain_identity_text = std::move(identity_contents);
  ...
  for (const MusaBridgeComponentFingerprint& component : result.components) {
    if (component.name == "mcc" || component.name == "clang_offload_bundler" ||
        component.name == "llvm_pass_plugin") {
