...
absl::StatusOr<MainOptions> ParseCommandLine(int argc, char** argv) {
  ...
  const Flag flags[] = {
      ...
      {"llvm-pass-plugin", &options.paths.llvm_pass_plugin},
...
absl::StatusOr<MainOptions> ResolveAndValidatePaths(MainOptions options) {
  ...
  if (!options.paths.llvm_pass_plugin.empty()) {
    auto resolved = ResolvePath(options.paths.llvm_pass_plugin, "LLVM pass plugin",
                                /*directory=*/false, /*executable=*/false);
    if (!resolved.ok()) return resolved.status();
    options.paths.llvm_pass_plugin = *std::move(resolved);
    if (Dirname(options.paths.llvm_pass_plugin) !=
        Dirname(options.paths.bridge_executable)) {
      return absl::InvalidArgumentError("LLVM pass plugin escapes compiler bundle");
    }
  }
...
std::vector<std::string> RedactedPaths(const MainOptions& options) {
  return {Dirname(Dirname(options.paths.mcc)),
          ...
          options.paths.llvm_pass_plugin,
...
absl::StatusOr<MusaBridgeCompileResponse> CompileOneRequest(
  ...
  absl::StatusOr<MusaBridgeFingerprints> fingerprints =
      FingerprintMusaBridgeToolchain(
          options->paths,
          std::string(MccBundleProviderName(!options->paths.llvm_pass_plugin.empty())),
          std::string(MccBundleProviderCanonicalText(!options->paths.llvm_pass_plugin.empty())));
  if (!fingerprints.ok()) {
    return FailureResponse(request, RequestIdentity(request),
  ...
  codegen_options.llvm_pass_plugin_path = options->paths.llvm_pass_plugin;
  codegen_options.clang_offload_bundler_path =
      options->paths.clang_offload_bundler;
  codegen_options.temporary_directory_root = options->temporary_directory_root;
  codegen_options.architecture = request.architecture();
