...
MusaBridgeToolchainPaths ToolchainPaths() {
  ...
  paths.llvm_pass_plugin = RunfileFromEnvironment("MUSA_TEST_GUARD");
...
TEST(MusaCompilationProviderProcessTest,
  ...
  absl::StatusOr<MusaBridgeFingerprints> fingerprints =
      bridge::FingerprintMusaBridgeToolchain(
          bridge_paths, std::string(MccBundleProviderName(true)),
          std::string(MccBundleProviderCanonicalText(true)));
  ...
  paths.llvm_pass_plugin = bridge_paths.llvm_pass_plugin;
