...
TEST(MusaBridgeToolchainFingerprintTest, PassPluginChangesInvalidateIdentity) {
  FixturePaths fixture = MakeFixture();
  fixture.paths.llvm_pass_plugin = absl::StrCat(fixture.root, "/guard.so");
  Write(fixture.paths.llvm_pass_plugin, "guard-one");
  auto before = FingerprintMusaBridgeToolchain(fixture.paths, "mcc-bundle-v2",
                                               kProviderContract);
  ASSERT_TRUE(before.ok()) << before.status();
  EXPECT_EQ(before->components.size(), 11);
  Write(fixture.paths.llvm_pass_plugin, "guard-two");
  auto after = FingerprintMusaBridgeToolchain(fixture.paths, "mcc-bundle-v2",
                                              kProviderContract);
  ASSERT_TRUE(after.ok()) << after.status();
  EXPECT_NE(before->provider_fingerprint, after->provider_fingerprint);
  EXPECT_NE(before->toolchain_fingerprint, after->toolchain_fingerprint);
  EXPECT_EQ(before->bridge_fingerprint, after->bridge_fingerprint);
}
