...
TEST_F(MusaMuFftApiTest, DefaultSnapshotOmitsUnqualifiedFftWithoutEnablingIt) {
  g_version = {/*major=*/1, /*minor=*/12, /*patch=*/0};
  auto api = CreateApi(
      std::make_unique<FakeSymbolLoader>(reinterpret_cast<void*>(&FakeGetter)));
  ASSERT_OK_AND_ASSIGN(auto available, api->AvailableOptionalAbis());
  EXPECT_TRUE(available.empty());
  EXPECT_THAT(api->AvailableOptionalAbis(/*explicitly_configured=*/true),
              StatusIs(absl::StatusCode::kFailedPrecondition,
                       HasSubstr("unqualified muFFT version 1.12.0")));
  void* plan = nullptr;
  EXPECT_THAT(api->Create(&plan),
              StatusIs(absl::StatusCode::kFailedPrecondition));
  EXPECT_EQ(calls_.create, 0);

  api_table_.abi_version = 99;
  auto malformed = CreateApi(
      std::make_unique<FakeSymbolLoader>(reinterpret_cast<void*>(&FakeGetter)));
  EXPECT_THAT(malformed->AvailableOptionalAbis(),
              StatusIs(absl::StatusCode::kFailedPrecondition,
                       HasSubstr("ABI version 99")));
}
