...
constexpr XlaMusaMuDnnVersion kQualifiedMuDnnVersion28 = {
    /*major=*/2, /*minor=*/8, /*patch=*/0};
constexpr XlaMusaMuDnnVersion kQualifiedMuDnnVersion33 = {
...
std::unique_ptr<internal::MusaSymbolLoader> CreateDefaultShimLoader() {
  ...
  return internal::CreateMusaDsoLoader(
      {(*runtime)->Path(kMusaMuDnnShimSoname)});
}
...
absl::Status ValidateVersion(const XlaMusaMuDnnVersion& version,
                             absl::string_view loaded_path) {
  const bool is_qualified_28 =
      version.major == kQualifiedMuDnnVersion28.major &&
      version.minor == kQualifiedMuDnnVersion28.minor &&
      version.patch == kQualifiedMuDnnVersion28.patch;
  const bool is_qualified_33 =
      version.major == kQualifiedMuDnnVersion33.major &&
      version.minor == kQualifiedMuDnnVersion33.minor &&
      version.patch == kQualifiedMuDnnVersion33.patch;
  if (!is_qualified_28 && !is_qualified_33) {
    return absl::FailedPreconditionError(absl::StrCat(
        "muDNN shim ", loaded_path, " reports unqualified muDNN version ",
        version.major, ".", version.minor, ".", version.patch,
        "; expected 2.8.0 or 3.3.0"));
  }
  return absl::OkStatus();
}
...
absl::Status MusaMuDnnApi::Initialize() const {
  ...
  ABSL_RETURN_IF_ERROR(MuDnnStatus(api->get_version(&version), "get_version"));
  ABSL_RETURN_IF_ERROR(ValidateVersion(version, loader_->loaded_path()));
...
absl::Status MusaMuDnnApi::CreateHandle(int32_t device_ordinal,
  ...
  *handle = nullptr;
  ...
  ABSL_RETURN_IF_ERROR(MuDnnStatus(api_->create_handle(device_ordinal, handle),
                              "create_handle"));
  if (*handle == nullptr) {
...
absl::Status MusaMuDnnApi::CreateTensor(void** tensor) const {
  ...
  *tensor = nullptr;
  ABSL_RETURN_IF_ERROR(MuDnnStatus(api_->create_tensor(tensor), "create_tensor"));
  if (*tensor == nullptr) {
...
absl::Status MusaMuDnnApi::ConfigureTensor(void* tensor,
  ...
  ABSL_RETURN_IF_ERROR(ValidateDataType(data_type));
  ABSL_RETURN_IF_ERROR(ValidateTensorFormat(format));
  if (rank < 3 || rank > 5) {
    return absl::InvalidArgumentError(
...
absl::Status MusaMuDnnApi::GetWorkspaceSize(
  ...
  ABSL_RETURN_IF_ERROR(ValidateConvolutionArguments(handle, convolution, kind,
                                               output, data, filter));
  ABSL_RETURN_IF_ERROR(ValidateAlgorithm(algorithm, /*allow_recommended=*/true));
...
absl::Status MusaMuDnnApi::Convolve(
  ...
  ABSL_RETURN_IF_ERROR(ValidateConvolutionArguments(handle, convolution, kind,
                                               output, data, filter));
  ABSL_RETURN_IF_ERROR(ValidateAlgorithm(algorithm, /*allow_recommended=*/true));
  ABSL_RETURN_IF_ERROR(ValidateWorkspaceAllocator(workspace_allocator));
