...
std::atomic<bool>* ProcessExiting() {
  static auto* process_exiting = new std::atomic<bool>(false);
  return process_exiting;
}

void MarkProcessExiting() {
  ProcessExiting()->store(true, std::memory_order_release);
}

void RegisterProcessExitHandler() {
  static const bool registered = [] {
    if (std::atexit(MarkProcessExiting) != 0) {
      LOG(WARNING) << "Unable to register the MUSA process-exit handler";
      return false;
    }
    return true;
  }();
...
absl::StatusOr<std::unique_ptr<MusaContext>> MusaContext::Create(
  ...
  ABSL_ASSIGN_OR_RETURN(MUdevice device, driver->Device(device_ordinal));
  ABSL_ASSIGN_OR_RETURN(MusaPrimaryContextState state,
  ...
  if (state.flags != kDesiredFlags) {
    if (state.active) {
      ...
      ABSL_RETURN_IF_ERROR(driver->SetPrimaryContextFlags(device, kDesiredFlags));
    }
  }

  ABSL_ASSIGN_OR_RETURN(MUcontext context, driver->RetainPrimaryContext(device));
  if (context == nullptr) {
...
void MusaContext::SetActive() {
  absl::Status status = driver_->SetCurrentContext(context_);
  if (!status.ok() &&
      ProcessExiting()->load(std::memory_order_acquire) &&
      absl::IsFailedPrecondition(status)) {
    // MUSA's runtime exit handler can deinitialize the driver before XLA's
    // process-lifetime StreamExecutor singletons stop their status monitors.
    // A monitor activation racing that teardown must not abort an otherwise
    // successful process. Driver calls following this activation return their
    // ordinary failure status and drain outstanding callbacks.
    VLOG(1) << "Ignoring MUSA context activation during process teardown: "
            << status;
    return;
  }
  CHECK_OK(status);
