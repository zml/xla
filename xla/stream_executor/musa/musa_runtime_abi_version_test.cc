...
TEST(MusaRuntimeAbiVersionTest, AcceptsS4000TargetAndToolkit) {
  ASSERT_OK_AND_ASSIGN(
      MusaRuntimeAbiVersion runtime,
      MusaRuntimeAbiVersion::Create(
          SemanticVersion(5, 1, 0), SemanticVersion(5, 1, 0),
          SemanticVersion(5, 1, 0), SemanticVersion(5, 1, 0)));
  ExecutableAbiVersionProto proto = CompatibleExecutableProto();
  MusaExecutableVersion* musa = proto.mutable_musa_platform_version();
  musa->set_architecture(kS4000TargetArchitecture);
  musa->set_hardware_warp_size(kS4000HardwareWarpSize);
  musa->set_logical_subgroup_size(kS4000CompilerLogicalSubgroupSize);
  musa->set_required_toolkit_version("5.1.0");
  musa->set_minimum_runtime_version("5.1.0");
  musa->set_maximum_runtime_version("5.1.0");
  musa->set_minimum_driver_version("5.1.0");
  musa->set_maximum_driver_version("5.1.0");
  musa->set_minimum_kernel_driver_version("5.1.0");
  musa->set_maximum_kernel_driver_version("5.1.0");
  EXPECT_OK(CompatibilityStatus(runtime, proto));
}
...
TEST(MusaRuntimeAbiVersionTest, RejectsPlatformAndTargetMismatches) {
  ...
  const std::vector<std::pair<std::string, Mutation>> mutations = {
      ...
      {"architecture",
       [](MusaExecutableVersion* musa) { musa->set_architecture("mp_31"); }},
      {"features",
       [](MusaExecutableVersion* musa) {
