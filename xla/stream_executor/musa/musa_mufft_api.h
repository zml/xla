...
// adjacent DSO first and the versioned SONAME second for legacy diagnostics.
// The production default loader instead uses the adapter embedded in PJRT.
...
class MusaMuFftApi {
 public:
  ...
  // Default discovery omits a well-formed but unqualified vendor version.
  // Explicit paths and malformed ABI tables remain fail-closed. FFT dispatch
  // itself always requires Init() to succeed.
  absl::StatusOr<std::vector<MusaOptionalLibraryAbi>> AvailableOptionalAbis(
      bool explicitly_configured = false) const;
 ...
 private:
  ...
  mutable bool unqualified_vendor_version_ = false;
};
...
// produces an empty list. Default discovery also omits unqualified vendor
// versions; malformed tables and explicitly configured shims fail closed.
