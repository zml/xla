...
absl::StatusOr<MuBlasGemmTypes> GetMuBlasGemmTypes(
  ...
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuBlasDataType input_type, AsMuBlasDataType(type_a));
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuBlasDataType output_type, AsMuBlasDataType(type_c));
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuBlasComputeType compute_type,
...
absl::Status ValidatePointerArraySizes(size_t a_size, size_t b_size,
  ...
  ABSL_RETURN_IF_ERROR(ValidateBatch(batch_count));
...
MusaBlas::GetOrCreateHandleForNativeStream(void* native_stream) {
  ...
  std::unique_ptr<ActivateContext> activation = parent_->Activate();
  void* handle = nullptr;
  ABSL_RETURN_IF_ERROR(api_->Create(&handle));
...
absl::Status MusaBlas::DoBlasGemmInternal(
  ...
  ABSL_RETURN_IF_ERROR(ValidateAlgorithm(algorithm, *types, *api_, engine_options,
  ...
  ABSL_ASSIGN_OR_RETURN(std::shared_ptr<HandleState> state,
...
absl::Status MusaBlas::DoBlasScalInternal(Stream* stream, uint64_t n,
  ...
  std::unique_ptr<ActivateContext> activation = parent_->Activate();
  absl::MutexLock state_lock(&state->mu);
  ABSL_RETURN_IF_ERROR(api_->Scal(state->handle, scal_type, static_cast<int64_t>(n),
                             alpha, x->opaque(), incx));
  {
    absl::MutexLock lock(&handles_mu_);
    last_stream_ = state->native_stream;
...
absl::Status MusaBlas::DoBlasGemmBatchedInternal(
  ...
  ABSL_RETURN_IF_ERROR(
      ValidatePointerArraySizes(a.size(), b.size(), c.size(), batch_count));
  ABSL_RETURN_IF_ERROR(ValidateDimensions(transa, transb, m, n, k, lda, ldb, ldc));
  ...
  ABSL_ASSIGN_OR_RETURN(DeviceAddress<uint8_t> a_device,
                   scratch_allocator->AllocateBytes(pointer_bytes));
  ABSL_ASSIGN_OR_RETURN(DeviceAddress<uint8_t> b_device,
                   scratch_allocator->AllocateBytes(pointer_bytes));
  ABSL_ASSIGN_OR_RETURN(DeviceAddress<uint8_t> c_device,
                   scratch_allocator->AllocateBytes(pointer_bytes));
  ...
  ABSL_ASSIGN_OR_RETURN(std::unique_ptr<MemoryAllocation> host_allocation,
  ...
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuBlasOperation operation_a,
                   AsMuBlasOperation(transa));
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuBlasOperation operation_b,
                   AsMuBlasOperation(transb));
  ABSL_ASSIGN_OR_RETURN(std::shared_ptr<HandleState> state,
                   GetOrCreateHandle(stream));
  std::unique_ptr<ActivateContext> activation = parent_->Activate();
  absl::MutexLock state_lock(&state->mu);
  ABSL_RETURN_IF_ERROR(ConfigureAtomics(state.get(), engine_options,
                                   XLA_MUSA_MUBLAS_ALGORITHM_DEFAULT));
  ABSL_RETURN_IF_ERROR(api_->GemmBatched(
...
absl::Status MusaBlas::DoBlasGemmStridedBatchedInternal(
  ...
  ABSL_RETURN_IF_ERROR(ValidateBatch(batch_count));
  ABSL_RETURN_IF_ERROR(ValidateStrides(stride_a, stride_b, stride_c, batch_count));
  ABSL_RETURN_IF_ERROR(ValidateDimensions(transa, transb, m, n, k, lda, ldb, ldc));
  ABSL_ASSIGN_OR_RETURN(
      MuBlasGemmTypes types,
      GetMuBlasGemmTypes(type_a, type_b, type_c, computation_type));
  ...
  ABSL_ASSIGN_OR_RETURN(uint64_t a_elements,
                   RequiredMatrixElements(transa, m, k, lda));
  ABSL_ASSIGN_OR_RETURN(uint64_t b_elements,
                   RequiredMatrixElements(transb, k, n, ldb));
  ABSL_ASSIGN_OR_RETURN(
      uint64_t c_elements,
      RequiredMatrixElements(blas::Transpose::kNoTranspose, m, n, ldc));
  ...
  ABSL_ASSIGN_OR_RETURN(size_t element_size, DataTypeSize(type_a));
  ABSL_RETURN_IF_ERROR(ValidateBufferExtent(a, a_elements, stride_a, batch_count,
                                       element_size, "A"));
  ABSL_RETURN_IF_ERROR(ValidateBufferExtent(b, b_elements, stride_b, batch_count,
                                       element_size, "B"));
  ABSL_RETURN_IF_ERROR(ValidateBufferExtent(*c, c_elements, stride_c, batch_count,
                                       element_size, "C"));
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuBlasOperation operation_a,
                   AsMuBlasOperation(transa));
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuBlasOperation operation_b,
  ...
  ABSL_RETURN_IF_ERROR(api_->GemmStridedBatched(
      state->handle, types.input_type, types.output_type, types.compute_type,
      operation_a, operation_b, static_cast<int64_t>(m),
...
absl::Status MusaBlas::DoBlasTrsmInternal(
  ...
  ABSL_RETURN_IF_ERROR(ValidateTrsmDimensions(side, m, n, lda, ldb));
  if (m == 0 || n == 0) return absl::OkStatus();
  ...
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuBlasSide normalized_side, AsMuBlasSide(side));
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuBlasFill normalized_fill, AsMuBlasFill(uplo));
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuBlasOperation normalized_transpose,
                   AsMuBlasOperation(transa));
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuBlasDiagonal normalized_diagonal,
                   AsMuBlasDiagonal(diag));
  ABSL_ASSIGN_OR_RETURN(std::shared_ptr<HandleState> state,
                   GetOrCreateHandle(stream));
  std::unique_ptr<ActivateContext> activation = parent_->Activate();
  absl::MutexLock state_lock(&state->mu);
  ABSL_RETURN_IF_ERROR(api_->Trsm(
...
absl::Status MusaBlas::DoBlasTrsmBatchedInternal(
  ...
  ABSL_RETURN_IF_ERROR(ValidateTrsmPointerArray(as, batch_count, "A"));
  ABSL_RETURN_IF_ERROR(ValidateTrsmPointerArray(*bs, batch_count, "B"));
  ...
  std::unique_ptr<ActivateContext> activation = parent_->Activate();
  absl::MutexLock state_lock(&state->mu);
  ABSL_RETURN_IF_ERROR(api_->TrsmBatched(
      state->handle, trsm_type, normalized_side, normalized_fill,
      normalized_transpose, normalized_diagonal, static_cast<int64_t>(m),
