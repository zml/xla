...
absl::Status ValidateMusaExecutableAbi(
  ...
  if (musa.target_triple() != kMusaTargetTriple ||
      ...
      !IsQualifiedMusaAbiTarget(musa.architecture(),
                                musa.hardware_warp_size(),
                                musa.logical_subgroup_size())) {
    return InvalidAbi("target ABI does not match a qualified MUSA target");
  }
