...
TEST(MusaExecutorTest, QualifiedDeviceDescriptionUsesLiveQueries) {
  ...
  const bool is_s4000 = description->name() == "MTT S4000";
  EXPECT_TRUE(is_s4000 || description->name() == "MTT S80");
  ...
  EXPECT_EQ(description->compile_time_toolkit_version(),
            (is_s4000 ? SemanticVersion{5, 1, 0} : SemanticVersion{4, 0, 1}));
  ...
  EXPECT_EQ(capability->architecture(), is_s4000 ? "mp_22" : "mp_21");
  EXPECT_EQ(capability->major(), 2);
  EXPECT_EQ(capability->minor(), is_s4000 ? 2 : 1);
...
TEST(MusaExecutorTest, DeviceAllocationRetainsContextPastExecutorLifetime) {
  ...
  std::unique_ptr<MemoryAllocation> allocation;
  {
    ...
    ASSERT_OK_AND_ASSIGN(allocator,
                         executor.CreateMemoryAllocator(MemorySpace::kDevice));
    ASSERT_OK_AND_ASSIGN(allocation, allocator->Allocate(256));
...
TEST(MusaExecutorTest, DeviceAllocatorAlignsLiveRegionsAndPreservesContents) {
  MusaExecutor executor(/*platform=*/nullptr, /*device_ordinal=*/0);
  ...
  for (int repeat = 0; repeat < 2; ++repeat) {
    for (uint64_t size : {UINT64_C(1), UINT64_C(4), UINT64_C(32), UINT64_C(256),
                          UINT64_C(4096), UINT64_C(2097152)}) {
      ...
      EXPECT_EQ(reinterpret_cast<uintptr_t>(address.opaque()) % 256, 0);
      EXPECT_EQ(address.size(), size);
      contents.emplace_back(size, static_cast<uint8_t>(allocations.size() + 1));
      ASSERT_OK(
          executor.SynchronousMemcpy(&address, contents.back().data(), size));
      allocations.push_back(std::move(allocation));
    }
  }
  for (size_t index = 0; index < allocations.size(); ++index) {
    std::vector<uint8_t> actual(contents[index].size());
    ASSERT_OK(executor.SynchronousMemcpy(
        actual.data(), allocations[index]->address(), actual.size()));
    EXPECT_EQ(actual, contents[index]);
  }
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<MemoryAllocation> empty,
                       allocator->Allocate(0));
  EXPECT_TRUE(empty->address().is_null());
  EXPECT_EQ(empty->address().size(), 0);
  EXPECT_EQ(
      allocator->Allocate(std::numeric_limits<uint64_t>::max()).status().code(),
      absl::StatusCode::kResourceExhausted);
}

TEST(MusaExecutorTest, LegacyDeviceAllocationsAreAlignedAndIndependent) {
  MusaExecutor executor(/*platform=*/nullptr, /*device_ordinal=*/0);
  ASSERT_OK(executor.Init());
  std::vector<DeviceAddressBase> allocations;
  for (uint64_t size : {1, 4, 128, 257, 4096}) {
    DeviceAddressBase address =
        executor.Allocate(size, static_cast<int64_t>(MemorySpace::kDevice));
    ASSERT_FALSE(address.is_null());
    EXPECT_EQ(reinterpret_cast<uintptr_t>(address.opaque()) % 256, 0);
    EXPECT_EQ(address.size(), size);
    uint8_t value = static_cast<uint8_t>(allocations.size() + 1);
    ASSERT_OK(executor.SynchronousMemcpy(&address, &value, 1));
    allocations.push_back(address);
  }
  for (size_t i = 0; i < allocations.size(); ++i) {
    uint8_t value = 0;
    ASSERT_OK(executor.SynchronousMemcpy(&value, allocations[i], 1));
    EXPECT_EQ(value, i + 1);
    executor.Deallocate(&allocations[i]);
    EXPECT_TRUE(allocations[i].is_null());
  }
}

TEST(MusaExecutorTest, AlignedBfcPoolRetainsContextPastExecutorLifetime) {
  std::unique_ptr<tsl::BFCAllocator> pool;
  std::vector<void*> allocations;
  {
    ...
    ASSERT_OK_AND_ASSIGN(std::unique_ptr<MemoryAllocator> allocator,
                         executor.CreateMemoryAllocator(MemorySpace::kDevice));
    auto sub_allocator = std::make_unique<StreamExecutorAllocator>(
        std::move(allocator), MemorySpace::kDevice, 0);
    pool = std::make_unique<tsl::BFCAllocator>(
        std::move(sub_allocator), 8 * 1024 * 1024, "musa_alignment_test",
        tsl::BFCAllocator::Options{});
    for (size_t size : {size_t{1}, size_t{4}, size_t{255}, size_t{257},
                        size_t{4096}, size_t{2 * 1024 * 1024}}) {
      void* pointer = pool->AllocateRaw(256, size);
      ASSERT_NE(pointer, nullptr);
      EXPECT_EQ(reinterpret_cast<uintptr_t>(pointer) % 256, 0);
      allocations.push_back(pointer);
    }
  }
  for (void* pointer : allocations) pool->DeallocateRaw(pointer);
  pool.reset();
  EXPECT_TRUE(MusaDriver::Instance().CurrentContext().ok());
}
