...
std::unique_ptr<MusaSymbolLoader> CreateMusaDsoLoader(
  ...
  // vendor installer used on qualified MUSA hosts can instead leave DSOs only
...
std::unique_ptr<MusaSymbolLoader> CreateMusaDriverDsoLoader() {
  // SDK 5 uses libmusa.so.1. Prefer its stable SONAME so a relocatable
  // runtime bundle and the driver API table resolve to the same loaded DSO.
  // Loading the host's unversioned libmusa.so alongside a bundled .so.1
  // creates two independent driver states and breaks initialization on S4000.
  return CreateMusaDsoLoader(
      {"libmusa.so.1", "libmusa.so.1.5", "libmusa.so"});
}
