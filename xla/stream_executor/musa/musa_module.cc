...
absl::StatusOr<std::shared_ptr<MusaModule>> MusaModule::Load(
  ...
  ABSL_ASSIGN_OR_RETURN(MusaMubinMetadata metadata, ValidateMusaMubin(mubin));
  std::shared_ptr<const std::vector<uint8_t>> bytes =
...
absl::StatusOr<std::shared_ptr<MusaModule>> MusaModule::LoadValidated(
  ...
  MUmodule native_module;
  {
    ...
    ABSL_ASSIGN_OR_RETURN(native_module, driver->LoadModuleData(bytes->data()));
  }
...
absl::StatusOr<std::shared_ptr<MusaModule>> MusaModuleCache::GetOrLoadModule(
  ...
  ABSL_RETURN_IF_ERROR(ValidateLoadInputs(driver_, context_, architecture_,
                                     binary_abi_version_));
  ABSL_ASSIGN_OR_RETURN(MusaMubinMetadata metadata, ValidateMusaMubin(mubin));
