...
absl::Status MusaExecutor::Init() {
  ABSL_RETURN_IF_ERROR(driver_->Init());
  ABSL_ASSIGN_OR_RETURN(context_, MusaContext::Create(device_ordinal(), driver_));
  std::unique_ptr<ActivateContext> activation = Activate();
  ABSL_RETURN_IF_ERROR(MusaRuntime::Get()->Init());
  ABSL_RETURN_IF_ERROR(MusaRuntime::Get()->SetDevice(device_ordinal()));

  peer_access_cache_.clear();
  ABSL_ASSIGN_OR_RETURN(int device_count, driver_->DeviceCount());
  for (int peer_ordinal = 0; peer_ordinal < device_count; ++peer_ordinal) {
    ...
    ABSL_ASSIGN_OR_RETURN(MUdevice peer_device, driver_->Device(peer_ordinal));
...
absl::StatusOr<std::unique_ptr<Stream>> MusaExecutor::CreateStream(
  ...
  ABSL_ASSIGN_OR_RETURN(std::unique_ptr<MusaStream> stream,
...
absl::StatusOr<std::unique_ptr<Event>> MusaExecutor::CreateEvent() {
  ABSL_ASSIGN_OR_RETURN(MusaEvent event, MusaEvent::Create(this));
...
MusaExecutor::CreateCommandBuffer(CommandBuffer::Mode /*mode*/) {
  return absl::UnimplementedError("MUSA command buffers are not supported");
}
...
DeviceAddressBase MusaExecutor::Allocate(uint64_t size, int64_t memory_space) {
  ...
  auto allocator =
      CreateMemoryAllocator(static_cast<MemorySpace>(memory_space));
  if (!allocator.ok()) {
    LOG(ERROR) << "MUSA allocator creation failed: " << allocator.status();
    return DeviceAddressBase();
  }
  auto allocation = (*allocator)->Allocate(size);
  if (!allocation.ok()) {
    LOG(ERROR) << "MUSA allocation failed: " << allocation.status();
    return DeviceAddressBase();
  }
  DeviceAddressBase address = (*allocation)->address();
  absl::MutexLock lock(&allocations_mu_);
  allocations_.emplace(address.opaque(), std::move(*allocation));
  return address;
}
      ...
      return std::make_unique<GenericMemoryAllocator>(
          [context = context_](uint64_t size)
            ...
            if (size >
              ...
              return absl::ResourceExhaustedError(
                  "MUSA device allocation alignment padding overflows");
            }
            gpu::ScopedActivateContext activation(context.get());
            ABSL_ASSIGN_OR_RETURN(void* ptr,
                             MusaRuntime::Get()->Malloc(
                                 size == 0 ? 0 : size + kAlignment - 1));
            // SDK allocations can be only 128-byte aligned. Keep the original
            // owner for freeing while exposing a 256-byte-aligned device view.
            const uintptr_t aligned =
                (reinterpret_cast<uintptr_t>(ptr) + kAlignment - 1) &
                ~(uintptr_t{kAlignment} - 1);
            return std::make_unique<GenericMemoryAllocation>(
                reinterpret_cast<void*>(aligned), size,
                [context, ptr](void*, uint64_t) {
                  gpu::ScopedActivateContext activation(context.get());
                  absl::Status status = MusaRuntime::Get()->Free(ptr);
                  if (!status.ok()) {
                    LOG(ERROR)
...
MusaExecutor::CreateDeviceDescription(int device_ordinal) {
  ...
  ABSL_ASSIGN_OR_RETURN(MUcontext previous_context, driver.CurrentContext());
  absl::StatusOr<std::unique_ptr<DeviceDescription>> discovered =
    ...
    ABSL_ASSIGN_OR_RETURN(int runtime_version,
                        MusaRuntime::Get()->RuntimeVersion());
    ABSL_ASSIGN_OR_RETURN(int driver_version, driver.DriverVersion());
    ...
    ABSL_ASSIGN_OR_RETURN(DeviceDescription description,
    ...
    ABSL_ASSIGN_OR_RETURN(MUdevice source_device, driver.Device(device_ordinal));
    ABSL_ASSIGN_OR_RETURN(int device_count, driver.DeviceCount());
    int active_mtlink_ports = 0;
    for (int peer_ordinal = 0; peer_ordinal < device_count; ++peer_ordinal) {
      if (peer_ordinal == device_ordinal) continue;
      ABSL_ASSIGN_OR_RETURN(MUdevice peer_device, driver.Device(peer_ordinal));
      ABSL_ASSIGN_OR_RETURN(MusaPeerAccessInfo peer,
...
absl::StatusOr<std::unique_ptr<Kernel>> MusaExecutor::LoadKernel(
  ...
  ABSL_ASSIGN_OR_RETURN(KernelMetadata metadata, kernel->GetKernelMetadata());
...
absl::StatusOr<DeviceAddressBase> MusaExecutor::GetSymbol(
  ...
  ABSL_ASSIGN_OR_RETURN(std::shared_ptr<MusaModule> module,
