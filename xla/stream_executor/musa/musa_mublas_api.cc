...
std::unique_ptr<internal::MusaSymbolLoader> CreateDefaultShimLoader() {
  ...
  return internal::CreateMusaDsoLoader(
      {(*runtime)->Path(kMusaMuBlasShimSoname)});
}
...
absl::Status MusaMuBlasApi::Initialize() const {
  ...
  if (v2_getter.ok() && *v2_getter != nullptr) {
    ...
    ABSL_RETURN_IF_ERROR(ValidateApiV2(api, loader_->loaded_path()));
    api_v2_ = api;
    return absl::OkStatus();
  }
  ...
  ABSL_RETURN_IF_ERROR(ValidateApiV1(api, loader_->loaded_path()));
  api_v1_ = api;
  return absl::OkStatus();
}
...
absl::Status MusaMuBlasApi::Create(void** handle) const {
  ...
  *handle = nullptr;
  ...
  ABSL_RETURN_IF_ERROR(MuBlasStatus(create(handle), "create"));
  if (*handle == nullptr) {
...
absl::Status MusaMuBlasApi::Scal(void* handle, XlaMusaMuBlasScalType scal_type,
  ...
  ABSL_RETURN_IF_ERROR(ValidateNormalizedScalType(scal_type));
  if (!SupportsScal()) {
    return absl::UnimplementedError(
...
absl::Status MusaMuBlasApi::GemmWithAlgorithm(
  ...
  ABSL_RETURN_IF_ERROR(ValidateNormalizedAlgorithm(algorithm, input_type,
