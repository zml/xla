...
absl::Status PreloadMusaMuFftClosure(
    absl::string_view shim_path, const std::vector<std::string>& closure) {
...
absl::Status PreloadMusaMuFftAdjacentClosure(absl::string_view shim_path) {
  auto closure = GetMusaMuFftAdjacentClosurePathsImpl(shim_path);
  if (!closure.ok()) return closure.status();
  return PreloadMusaMuFftClosure(shim_path, *closure);
}
...
std::unique_ptr<internal::MusaSymbolLoader> CreateDefaultShimLoader() {
  ...
  if (configured_path.has_value()) {
    return CreateShimLoader(configured_path, {});
  }
  auto runtime = internal::GetMusaEmbeddedRuntime();
  if (!runtime.ok()) {
    return std::make_unique<ErrorMusaSymbolLoader>(runtime.status());
  }
  const std::string shim = (*runtime)->Path(kMusaMuFftShimSoname);
  const std::string libraries = JoinPath((*runtime)->sdk_root(), "lib");
  absl::Status status = PreloadMusaMuFftClosure(
      shim, {
                JoinPath(libraries, "libmusart.so.5"),
                JoinPath(libraries, "libmtfft-device-1.so"),
                JoinPath(libraries, "libmtfft-device-2.so"),
                JoinPath(libraries, "libmtfft-device-3.so"),
                JoinPath(libraries, "libmtfft-device-0.so"),
            });
  if (!status.ok()) {
    return std::make_unique<ErrorMusaSymbolLoader>(status);
  }
  return internal::CreateMusaDsoLoader({shim});
}
...
absl::Status MusaMuFftApi::Initialize() const {
  ...
  ABSL_RETURN_IF_ERROR(loader_->Load());
  ...
  ABSL_RETURN_IF_ERROR(ValidateApi(api, loader_->loaded_path()));

  XlaMusaMuFftVersion version = {};
  ABSL_RETURN_IF_ERROR(MuFftStatus(api->get_version(&version), "get_version"));
  const absl::Status version_status =
      ValidateVersion(version, loader_->loaded_path());
  if (!version_status.ok()) {
    unqualified_vendor_version_ = true;
...
absl::Status MusaMuFftApi::Create(void** plan) const {
  ...
  *plan = nullptr;
  ABSL_RETURN_IF_ERROR(MuFftStatus(api_->create(plan), "create"));
  if (*plan == nullptr) {
...
absl::Status MusaMuFftApi::MakePlanMany(
  ...
  *workspace_size_bytes = 0;
  ...
  ABSL_RETURN_IF_ERROR(ValidateType(type));
  return MuFftStatus(
...
absl::Status MusaMuFftApi::ExecC2C(void* plan, void* input, void* output,
  ...
  ABSL_RETURN_IF_ERROR(Init());
  ABSL_RETURN_IF_ERROR(ValidateExecutionArguments(plan, input, output));
  ABSL_RETURN_IF_ERROR(ValidateDirection(direction));
...
GetAvailableMusaMuFftOptionalLibraryAbis() {
  const char* configured = std::getenv(kMusaMuFftShimEnvironment);
  return GetMusaMuFftApi()->AvailableOptionalAbis(
      configured != nullptr && configured[0] != '\0');
}

absl::StatusOr<std::vector<MusaOptionalLibraryAbi>>
MusaMuFftApi::AvailableOptionalAbis(bool explicitly_configured) const {
  const absl::Status status = Init();
  if (absl::IsNotFound(status) ||
      (!explicitly_configured && unqualified_vendor_version_)) {
