...
absl::Status MusaKernel::Launch(const ThreadDim& thread_dims,
  ...
  if (const auto* packed = DynCast<KernelArgsPackedArrayBase>(&args)) {
    ...
    ABSL_ASSIGN_OR_RETURN(std::unique_ptr<KernelArgsPackedArrayBase> repacked,
                        pack(*this, *packed));
  ...
  if (const auto* device_addresses =
    ...
    ABSL_ASSIGN_OR_RETURN(std::unique_ptr<KernelArgsPackedArrayBase> packed,
