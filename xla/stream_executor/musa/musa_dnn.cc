  ...
  switch (type) {
    ...
    default:
      return absl::UnimplementedError(absl::StrCat(
          "qualified muDNN convolution supports only f16 and f32; got DNN "
          "type ",
...
absl::StatusOr<TensorConfig> BatchTensorConfig(
  ...
  ABSL_RETURN_IF_ERROR(ValidateTensorConfig(config, "muDNN batch tensor"));
  return config;
}
...
absl::StatusOr<TensorConfig> FilterTensorConfig(
  ...
  switch (layout) {
    ...
    case dnn::FilterLayout::kOutputYXInput:
      return absl::UnimplementedError(
          "qualified muDNN requires contiguous filters: XLA's channel-last "
  ...
  ABSL_RETURN_IF_ERROR(ValidateTensorConfig(config, "muDNN filter tensor"));
  return config;
}
...
class ScopedMuDnnTensor {
 public:
  static absl::StatusOr<std::unique_ptr<ScopedMuDnnTensor>> Create(
    ...
    void* handle = nullptr;
    ABSL_RETURN_IF_ERROR(api->CreateTensor(&handle));
    auto tensor =
...
class ScopedMuDnnConvolution {
 public:
  static absl::StatusOr<std::unique_ptr<ScopedMuDnnConvolution>> Create(
    ...
    void* handle = nullptr;
    ABSL_RETURN_IF_ERROR(api->CreateConvolution(&handle));
    auto convolution = std::unique_ptr<ScopedMuDnnConvolution>(
        new ScopedMuDnnConvolution(api, handle));
    ABSL_RETURN_IF_ERROR(api->ConfigureConvolution(
...
class MusaConvRunner final : public dnn::ConvRunner {
 public:
  static absl::StatusOr<std::unique_ptr<MusaConvRunner>> Create(
    ...
    ABSL_ASSIGN_OR_RETURN(XlaMusaMuDnnDataType type, ToMuDnnDataType(input_type));
    ABSL_ASSIGN_OR_RETURN(TensorConfig input_config,
                        BatchTensorConfig(input_descriptor));
    ABSL_ASSIGN_OR_RETURN(TensorConfig filter_config,
                        FilterTensorConfig(filter_descriptor));
    ABSL_ASSIGN_OR_RETURN(TensorConfig output_config,
                        BatchTensorConfig(output_descriptor));
    ABSL_ASSIGN_OR_RETURN(std::unique_ptr<ScopedMuDnnTensor> input,
                        ScopedMuDnnTensor::Create(api, type, input_config));
    ABSL_ASSIGN_OR_RETURN(std::unique_ptr<ScopedMuDnnTensor> filter,
                        ScopedMuDnnTensor::Create(api, type, filter_config));
    ABSL_ASSIGN_OR_RETURN(std::unique_ptr<ScopedMuDnnTensor> output,
  ...
  absl::Status QueryWorkspace(Stream* stream, DeviceAddressBase input_data,
    ...
    ABSL_RETURN_IF_ERROR(PrepareHandle(stream));
    ABSL_RETURN_IF_ERROR(SetAddresses(input_data, filter_data, output_data));
    TensorRoles roles = Roles();
    uint64_t workspace_size = 0;
    ABSL_RETURN_IF_ERROR(api_->GetWorkspaceSize(
  ...
  absl::Status operator()(Stream* stream,
    ...
    ABSL_RETURN_IF_ERROR(api_->Convolve(handle_, convolution_->handle(), MuDnnKind(),
 ...
 private:
  ...
  absl::Status SetAddresses(DeviceAddressBase input_data,
    ...
    ABSL_RETURN_IF_ERROR(input_->SetAddress(input_data));
    ABSL_RETURN_IF_ERROR(filter_->SetAddress(filter_data));
...
absl::Status MusaDnn::Init() {
  ...
  ABSL_RETURN_IF_ERROR(api_->Init());
  absl::MutexLock lock(&mutex_);
...
absl::StatusOr<dnn::VersionInfo> MusaDnn::GetVersion() {
  ...
  ABSL_RETURN_IF_ERROR(api_->GetVersion(&version));
...
absl::Status MusaDnn::GetConvolveRunners(
  ...
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuDnnConvolutionKind mudnn_kind,
  ...
  if (engine_options.require_determinism) {
    return absl::UnimplementedError(
        "qualified muDNN exposes no algorithm determinism metadata; "
  ...
  for (size_t index = 0; index < algorithm_count; ++index) {
    ...
    ABSL_RETURN_IF_ERROR(workspace_status);
...
MusaDnn::ConvolveRunnerFromDesc(
  ...
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuDnnConvolutionKind mudnn_kind,
                      ToMuDnnConvolutionKind(kind));
  (void)mudnn_kind;
  ABSL_ASSIGN_OR_RETURN(XlaMusaMuDnnAlgorithm algorithm,
