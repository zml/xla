...
#if MUSART_VERSION >= 50000
// SDK 5.x removed the legacy host aliases while retaining the same exported C
// ABI: integer parameters are 32-bit `int` and half values are 16 raw bits.
using mublas_int = int;
using mublas_half = uint16_t;
static_assert(sizeof(mublas_int) == sizeof(int32_t));
static_assert(sizeof(mublas_half) == 2);
#endif

// The qualified host headers expose mublasHgemm only to __MUSACC__, although
...
XlaMusaMuBlasStatus ToShimStatus(mublasStatus status) {
  ...
  // SDK 5.1 aliases several invalid-argument enumerators to the same value.
  if (status == MUBLAS_STATUS_INVALID_HANDLE ||
      status == MUBLAS_STATUS_INVALID_POINTER ||
      status == MUBLAS_STATUS_INVALID_SIZE ||
      status == MUBLAS_STATUS_INVALID_VALUE) {
    return XLA_MUSA_MUBLAS_STATUS_INVALID_ARGUMENT;
  }
  if (status == MUBLAS_STATUS_NOT_IMPLEMENTED) {
    return XLA_MUSA_MUBLAS_STATUS_NOT_SUPPORTED;
  }
  if (status == MUBLAS_STATUS_MEMORY_ERROR) {
...
XlaMusaMuBlasStatus Create(void** handle) {
  ...
  *handle = nullptr;
  ...
  status = mublasSetMathMode(native_handle, MUBLAS_DEFAULT_MATH);
  if (status != MUBLAS_STATUS_SUCCESS) {
    mublasDestroy(native_handle);
    return ToShimStatus(status);
  }
