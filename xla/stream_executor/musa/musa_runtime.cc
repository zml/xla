...
std::unique_ptr<internal::MusaSymbolLoader> CreateRuntimeLoader() {
  ...
  return internal::CreateMusaDsoLoader(
      {absl::StrCat((*runtime)->sdk_root(), "/lib/libmusart.so.5")},
...
MusaRuntime* MusaRuntime::Get() {
  static auto* runtime = new MusaRuntime(CreateRuntimeLoader());
  return runtime;
}
...
#define XLA_MUSA_ASSIGN_ATTRIBUTE(field, attribute)                   \
  ...
  int ecc_enabled = 0;
  ABSL_ASSIGN_OR_RETURN(ecc_enabled,
