...
MusaDeviceProperties S4000Properties() {
  MusaDeviceProperties properties = S80Properties();
  properties.name = "MTT S4000";
  properties.compute_capability_minor = 2;
  return properties;
}
...
TEST(MusaDeviceDescriptionTest, BuildsMeasuredS80Contract) {
  ...
  EXPECT_EQ(description.fpus_per_core(), -1);
...
TEST(MusaDeviceDescriptionTest, BuildsS4000Contract) {
  MusaDeviceVersions versions{
      .runtime_api = 50100,
      .driver_api = 50100,
      .compile_time_toolkit = 50100,
      .kernel_mode_driver = SemanticVersion{5, 1, 0},
  };
  ASSERT_OK_AND_ASSIGN(
      DeviceDescription description,
      BuildMusaDeviceDescription(S4000Properties(), versions));

  EXPECT_EQ(description.name(), "MTT S4000");
  EXPECT_EQ(description.compile_time_toolkit_version(),
            (SemanticVersion{5, 1, 0}));
  EXPECT_EQ(description.threads_per_warp(), 128);
  EXPECT_EQ(description.fpus_per_core(), 128);
  const MusaComputeCapability* capability =
      description.gpu_compute_capability().musa_compute_capability();
  ASSERT_NE(capability, nullptr);
  EXPECT_EQ(capability->architecture(), "mp_22");
  EXPECT_EQ(capability->major(), 2);
  EXPECT_EQ(capability->minor(), 2);
  EXPECT_EQ(capability->hardware_warp_size(), 128);
  EXPECT_EQ(capability->logical_subgroup_size(), 32);
}
...
TEST(MusaDeviceDescriptionTest, RejectsInvalidComputeCapability) {
  ...
  EXPECT_THAT(MusaArchitectureFromComputeCapability(2, 1),
              IsOkAndHolds("mp_21"));
  EXPECT_THAT(MusaArchitectureFromComputeCapability(2, 2),
              IsOkAndHolds("mp_22"));
}
