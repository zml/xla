...
absl::StatusOr<std::string> MusaArchitectureFromComputeCapability(int major,
  ...
  if (major == kS80ComputeCapabilityMajor &&
    ...
    return kS80TargetArchitecture;
  }
  if (major == kS4000ComputeCapabilityMajor &&
      minor == kS4000ComputeCapabilityMinor) {
    return kS4000TargetArchitecture;
  }
  return absl::FailedPreconditionError(absl::StrCat(
      "Unqualified MUSA compute capability ", major, ".", minor,
      "; supported targets are S80 mp_21 and S4000 mp_22"));
}
...
absl::StatusOr<DeviceDescription> BuildMusaDeviceDescription(
  ...
  ABSL_ASSIGN_OR_RETURN(std::string architecture,
                   MusaArchitectureFromComputeCapability(
                       properties.compute_capability_major,
                       properties.compute_capability_minor));
  const std::string_view expected_device_name =
      QualifiedMusaDeviceName(architecture);
  const int expected_hardware_warp_size =
      QualifiedMusaHardwareWarpSize(architecture);
  const int expected_logical_subgroup_size =
      QualifiedMusaLogicalSubgroupSize(architecture);
  const int expected_toolkit_version =
      QualifiedMusaToolkitVersion(architecture);

  if (properties.name != expected_device_name) {
    return absl::FailedPreconditionError(
        absl::StrCat("Unqualified MUSA device '", properties.name,
                     "'; expected ", expected_device_name, " for ",
                     architecture));
  }
  ...
  ABSL_RETURN_IF_ERROR(ValidatePositive(properties.max_threads_per_block,
                                   "maximum threads per block"));
  ABSL_RETURN_IF_ERROR(ValidatePositive(properties.max_block_dim_x,
                                   "maximum block dimension X"));
  ABSL_RETURN_IF_ERROR(ValidatePositive(properties.max_block_dim_y,
                                   "maximum block dimension Y"));
  ABSL_RETURN_IF_ERROR(ValidatePositive(properties.max_block_dim_z,
  ...
  if (properties.hardware_warp_size != expected_hardware_warp_size) {
    return absl::FailedPreconditionError(absl::StrCat(
        "Unqualified MUSA hardware warp size: ", properties.hardware_warp_size,
        "; expected ", expected_hardware_warp_size, " for ", architecture));
  }
  ABSL_RETURN_IF_ERROR(ValidatePositive(properties.multiprocessor_count,
                                   "multiprocessor count"));
  ABSL_RETURN_IF_ERROR(ValidatePositive(properties.max_threads_per_multiprocessor,
                                   "maximum threads per multiprocessor"));
  ABSL_RETURN_IF_ERROR(ValidatePositive(properties.max_shared_memory_per_block,
                                   "shared memory per block"));
  ABSL_RETURN_IF_ERROR(
      ValidatePositive(properties.max_shared_memory_per_multiprocessor,
                       "shared memory per multiprocessor"));
  ABSL_RETURN_IF_ERROR(ValidatePositive(properties.max_registers_per_block,
                                   "registers per block"));
  ABSL_RETURN_IF_ERROR(ValidatePositive(properties.max_registers_per_multiprocessor,
  ...
  ABSL_RETURN_IF_ERROR(ValidatePositive(properties.texture_alignment_bytes,
                                   "texture alignment"));

  ABSL_ASSIGN_OR_RETURN(SemanticVersion runtime_version,
                   ParseMusaVersion(versions.runtime_api));
  ABSL_ASSIGN_OR_RETURN(SemanticVersion driver_version,
                   ParseMusaVersion(versions.driver_api));
  if (versions.compile_time_toolkit != expected_toolkit_version) {
    return absl::FailedPreconditionError(absl::StrCat(
        "Unqualified MUSA toolkit version ", versions.compile_time_toolkit,
        "; expected raw ", expected_toolkit_version, " for ", architecture));
  }
  ABSL_ASSIGN_OR_RETURN(SemanticVersion toolkit_version,
  ...
  description.set_gpu_compute_capability(
      GpuComputeCapability(MusaComputeCapability(
          ...
          expected_logical_subgroup_size)));
  ...
  if (architecture == kS4000TargetArchitecture) {
    description.set_fpus_per_core(kS4000FpusPerCore);
  }
