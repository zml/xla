...
absl::Status ValidateTargetContract(const MusaExecutableVersion& version) {
  ...
  if (version.target_triple() != kMusaTargetTriple ||
      ...
      !IsQualifiedMusaAbiTarget(version.architecture(),
                                version.hardware_warp_size(),
                                version.logical_subgroup_size())) {
    return Incompatible("target contract does not match a qualified MUSA ABI");
  }
  return absl::OkStatus();
}
...
MusaRuntimeAbiVersion::CreateFromApiVersions(
    ...
    std::vector<MusaOptionalLibraryAbi> available_optional_library_abis) {
  ABSL_ASSIGN_OR_RETURN(SemanticVersion parsed_runtime_version,
                   ParseMusaVersion(runtime_version));
  ABSL_ASSIGN_OR_RETURN(SemanticVersion parsed_driver_version,
                   ParseMusaVersion(driver_version));
  ABSL_ASSIGN_OR_RETURN(SemanticVersion parsed_toolkit_version,
...
absl::StatusOr<MusaRuntimeAbiVersion> MusaRuntimeAbiVersion::Create(
    ...
    std::vector<MusaOptionalLibraryAbi> available_optional_library_abis) {
  ABSL_RETURN_IF_ERROR(ValidateRuntimeFacts(runtime_version, driver_version,
...
absl::Status MusaRuntimeAbiVersion::IsCompatibleWith(
  ...
  ABSL_RETURN_IF_ERROR(ValidateRuntimeFacts(runtime_version_, driver_version_,
  ...
  ABSL_RETURN_IF_ERROR(ValidateTargetContract(musa));

  ABSL_ASSIGN_OR_RETURN(SemanticVersion required_toolkit,
  ...
  ABSL_RETURN_IF_ERROR(ValidateRange(runtime_version_,
                                musa.minimum_runtime_version(),
                                musa.maximum_runtime_version(), "runtime"));
  ABSL_RETURN_IF_ERROR(ValidateRange(driver_version_, musa.minimum_driver_version(),
                                musa.maximum_driver_version(), "driver"));
  ABSL_RETURN_IF_ERROR(ValidateRange(
