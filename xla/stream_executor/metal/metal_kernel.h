...
class MetalKernel : public Kernel {
 public:
  ...
  // Like Launch, but dispatches INDIRECTLY off `indirect_grid_device_ptr` (a
  // device {gx,gy,gz} uint3 the GPU computed this step) instead of `block_dims`,
  // so the launched threadgroup count is a runtime value with no host read.
  // `block_dims` is kept only as the KPROF label's upper bound. Packed args only
  // (the device-address-array packing path is not needed by the indirect callers).
