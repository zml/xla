...
// METAL_BLIT_STATS=1: report blit traffic once a second on stderr. Every blit
// calls FlushConcurrentEncoder first, so a blit both moves bytes and splits the
// open compute encoder -- and an Instruments trace can show the resulting Blit
// intervals without saying what was copied or how much. Off by default.
