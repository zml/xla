...
TEST_F(TopkRewriterTest, RewriteWithReshapedIotaAtBatchOne) {
  // At batch 1 the index operand is spelled reshape(iota[N]) -> [1, N], not
  // broadcast(iota[N]) -> [B, N]: a broadcast that only adds size-1
  // dimensions is canonicalised to a reshape. Matching only the broadcast
  // form drops batch-1 decode off the TopK path and onto a full sort.
  for (std::string comparator :
       {getComparator(), getCompareComparator(), getStableComparator()}) {
    const std::string hlo_string = R"(
HloModule module
...
TEST_F(TopkRewriterTest, DeclinesAReshapeOfSomethingThatIsNotAnIota) {
  // The reshape form matches reshape(iota) specifically. A reshape of anything
  // else is not an index operand, whatever its shape.
  const std::string hlo_string = R"(
HloModule module
)" + getComparator() + R"(
ENTRY cluster {
  %arg_tuple.1 = f32[8,16] parameter(0)
  %iota.4 = s32[16]{0} iota(), iota_dimension=0
  %reshape.5 = s32[8,16]{1,0} broadcast(iota.4), dimensions={1}
  %bitcast.6 = s32[8,16]{1,0} reshape(s32[8,16]{1,0} %reshape.5)
  %sort.27 = (f32[8,16], s32[8,16]) sort(%arg_tuple.1, %bitcast.6),
    dimensions={1}, is_stable=true, to_apply=%compare
  %get-tuple-element.28 = f32[8,16] get-tuple-element(%sort.27), index=0
  ROOT %slice.29 = f32[8,5] slice(%get-tuple-element.28), slice={[0:8], [0:5]}
})";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          ParseAndReturnVerifiedModule(hlo_string));
  TopkRewriter rewriter([](const HloSortInstruction*, int64_t) { return true; });
  TF_ASSERT_OK_AND_ASSIGN(bool changed, rewriter.Run(module.get()));
