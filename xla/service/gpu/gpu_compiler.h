...
class GpuCompiler : public LLVMCompiler {
 ...
 protected:
  ...
  // Constructs the platform-specific executable ABI carried by the shared GPU
  // serialization format. Backends that need more than DeviceDescription can
  // override this without replacing GpuExecutable or PJRT AOT plumbing.
  ...
  // Selects the self-contained AOT format that serializes compiled thunks.
  virtual bool UseAotCompiledThunks(const HloModule& module) const;

  // Persistent kernel-cache entries must be bound to every target/compiler
  // identity that can change generated bytes. A backend may fail closed until
  // that complete identity is represented by the shared cache format.
  virtual absl::Status ValidatePersistentKernelCache(
