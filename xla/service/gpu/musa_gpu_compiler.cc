...
bool IsQualifiedMuDnnVersion(const se::dnn::VersionInfo& version) {
  return version == se::dnn::VersionInfo(2, 8, 0) ||
         version == se::dnn::VersionInfo(3, 3, 0);
}
...
bool IsQualifiedMuDnnCustomCall(const HloInstruction& instruction) {
  ...
  if (type == F16) {
    // The qualified muDNN f16 kernel is the contiguous, ungrouped 2D
...
absl::Status MusaGpuCompiler::OptimizeHloConvolutionCanonicalization(
  ...
  // unless the live StreamExecutor exposed a qualified 2.8.0 or 3.3.0
  // contract. The v1 shim does not expose algorithm determinism metadata, so
  // deterministic modules must also remain generic.
  const DebugOptions& debug_options = hlo_module->config().debug_options();
  if (debug_options.xla_gpu_experimental_disable_binary_libraries() ||
      debug_options.xla_gpu_deterministic_ops() ||
      debug_options.xla_gpu_exclude_nondeterministic_ops() ||
      !IsQualifiedMuDnnVersion(dnn_version) || is_deviceless) {
  ...
  ABSL_RETURN_IF_ERROR(probe_status);
...
absl::Status MusaGpuCompiler::AddConfigAssignerPass(
...
MusaGpuCompiler::CompileTargetBinary(
  ...
  const se::MusaComputeCapability& musa_compute_capability =
      device_description.musa_compute_capability();
  absl::StatusOr<musa::MusaLlvm14CompatibilityResult> compatible =
      musa::NormalizeMusaLlvmForLlvm14(
          *llvm_module, module_name, musa_compute_capability.architecture());
  if (!compatible.ok()) return compatible.status();

  ABSL_ASSIGN_OR_RETURN(musa::MusaCompilationOptions options,
...
MusaGpuCompiler::CreateExecutableAbiVersion(
  ...
  musa::MusaCompilationCapabilities capabilities =
      compilation_provider_->capabilities();
  capabilities.target = absl::StrCat(
      stream_executor::musa::kMusaTargetTriple, "/",
      device_description.musa_compute_capability().architecture());
  return musa::BuildMusaExecutableEnvelope(
      device_description, compilation_provider_->identity(),
      capabilities, options, main_binary,
