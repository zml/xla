// MLX tiled quantized Steel GEMM, adapted for DeepSeek-style 128x128 block FP8.
//
// Vendored/adapted from MLX (MIT License, Copyright (c) 2023 Apple Inc.,
// https://github.com/ml-explore/mlx):
//   - the Steel GEMM types (BlockLoader / BaseMMAFrag / MMATile / BlockMMA and
//     supporting integral_constant / complex64_t / TransformNone helpers) are
//     copied VERBATIM from the already-vendored, compile-proven
//     mlx_steel_splitk.h (its mlx/backend/metal/kernels/{utils,steel/...} span).
//   - the q-GEMM impls (fp_qmm_t_impl, fp_gather_qmm_rhs) and the per-K-tile
//     gemm_loop_* helpers are adapted from
//     mlx/backend/metal/kernels/{fp_quantized,quantized_utils}.h.
//
// The only deviation from MLX's FP quantized path is the block-scale layout:
// MLX uses a per-(row, K-group) 1-D scale (one fp8/uint8 scale byte per group of
// `group_size` along K, per output row); DeepSeek uses one bf16 scale per
// 128x128 weight block, i.e. a 2-D [N/128, K/128] (resp. [E, N/128, K/128])
// bf16 scale tensor shared across 128 rows AND 128 K-columns. The adapted
// `Fp8BlockLoader` and the impl scale-base arithmetic encode that 2-D form.
//
// Self-contained: only <metal_*> system includes. Intended to be embedded as
// raw bytes and compiled at runtime, so it must not #include any project header.
...
// `METAL_FUNC` is provided by the Metal stdlib headers; define it defensively in
// case a toolchain does not (it expands to the same attributes MLX expects).
#ifndef METAL_FUNC
...
///////////////////////////////////////////////////////////////////////////////
// Per-K-tile gemm loop helpers (verbatim from MLX quantized_utils.h), needed by
// the gather kernel.
///////////////////////////////////////////////////////////////////////////////
