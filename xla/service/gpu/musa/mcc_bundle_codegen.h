...
// Closed configuration for the qualified MCC bundle route. Fields describing
// code generation are deliberately validated against the S80/S4000 contract
// rather than accepted as general MCC flags.
struct MccBundleCodegenOptions {
  ...
  std::string llvm_pass_plugin_path;
...
absl::string_view MccBundleProviderName(bool use_pass_guard = false);
absl::string_view MccBundleProviderCanonicalText(bool use_pass_guard = false);
