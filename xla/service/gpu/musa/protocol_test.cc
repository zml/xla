...
TEST(MusaBridgeProtocolTest, RequestCanonicalRoundTripIsStable) {
  ...
  EXPECT_EQ(MusaBridgeSha256Hex(*first),
            "cbc80d5d38273930cef8e1dbf62a7e9b003dee8cd4693e12d46573702dfce853");
}
...
TEST(MusaBridgeProtocolTest, ResponseCanonicalRoundTripPreservesBinaryMubin) {
  ...
  EXPECT_EQ(MusaBridgeSha256Hex(*wire),
            "5e0e8939f2fd564050240659203333d77c8e88e74378230c47e5e8fe8365da90");
}
...
TEST(MusaBridgeProtocolTest, AcceptsS4000Architecture) {
  MusaBridgeCompileRequest request = MakeValidRequest();
  request.set_architecture(kMusaS4000TargetArchitecture);
  EXPECT_TRUE(ValidateMusaBridgeCompileRequest(request).ok());
}
