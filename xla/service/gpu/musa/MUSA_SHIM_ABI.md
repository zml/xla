...
attributes, vendor intrinsic, minimum mapping revision, and target set.
...
v1 currently uses mapping version 4 and the canonical mapping SHA-256 in
`musa_shim_abi.h`.
...
The `llvm.musa.barrier0` mapping is available on `mp_21` and `mp_22`; it is
convergent and has unrestricted read/write effects so memory operations cannot
move across it.
...
Mapping version 4 keeps the same bounded instruction and address-space
contract and admits the S4000 `mp_22` compiler target. The request architecture
is forwarded to `mcc`, included in bridge identity, and independently validated
