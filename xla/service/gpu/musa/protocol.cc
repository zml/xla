...
absl::Status ValidateMusaBridgeCompileRequest(
  ...
  if (!IsSupportedMusaTargetArchitecture(request.architecture())) {
    return InvalidField("architecture",
                        "is not a qualified mp_21 or mp_22 target");
  }
  ...
  if (request.data_layout() != kMusaDataLayout) {
    return InvalidField("data_layout", "does not match the qualified layout");
  }
