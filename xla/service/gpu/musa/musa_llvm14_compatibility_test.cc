...
TEST(MusaLlvm14CompatibilityTest, PreservesS4000ArchitectureMetadata) {
  absl::StatusOr<MusaLlvm14CompatibilityResult> result =
      NormalizeMusaLlvmTextForLlvm14(
          Module(""), "s4000_architecture", kMusaS4000TargetArchitecture);
  ASSERT_THAT(result, IsOk());
  EXPECT_EQ(result->metadata.architecture, kMusaS4000TargetArchitecture);
  EXPECT_THAT(
      ValidateMusaBridgeIr(result->normalized_llvm, result->metadata), IsOk());
}
...
TEST(MusaLlvm14CompatibilityTest, NormalizesReviewedCosineExpAndLogProfiles) {
  const std::string text = Module(
      "  %cosine = call float @llvm.cos.f32(float 0.0)\n"
      "  %exponential = call float @llvm.exp.f32(float %cosine)\n"
      "  %logarithm = call float @llvm.log.f32(float %exponential)\n"
      "  %bits = bitcast float %logarithm to i32\n"
      "  store i32 %bits, ptr addrspace(1) %out, align 4",
  ...
  std::unique_ptr<llvm::Module> module =
      llvm::parseAssemblyString(text, diagnostic, context);
  ASSERT_NE(module, nullptr);
  for (absl::string_view name : {"llvm.cos.f32", "llvm.exp.f32", "llvm.log.f32"}) {
  ...
  EXPECT_THAT(normalized->normalized_llvm,
              HasSubstr("call float @llvm.log.f32"));
  EXPECT_FALSE(absl::StrContains(normalized->normalized_llvm, "memory("));
  EXPECT_THAT(
      ValidateMusaBridgeIr(normalized->normalized_llvm, normalized->metadata),
      IsOk());
...
TEST(MusaLlvm14CompatibilityTest, NormalizesScalarLogSubnormalsWithoutFlushing) {
  const std::string text = Module(
      "  %x = load float, ptr addrspace(1) %out, align 4\n"
      "  %v = call float @llvm.log.f32(float %x)\n"
      "  store float %v, ptr addrspace(1) %out, align 4",
      "declare float @llvm.log.f32(float)");
  auto normalized = NormalizeMusaLlvmTextForLlvm14(text, "log_subnormals");
  ASSERT_THAT(normalized, IsOk());
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("log_is_subnormal"));
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("uitofp i32"));
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("log_subnormal_result"));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("fmul")));
  EXPECT_THAT(
      ValidateMusaBridgeIr(normalized->normalized_llvm, normalized->metadata),
      IsOk());
}

TEST(MusaLlvm14CompatibilityTest, RejectsUnreviewedLogCallForms) {
  for (const char* call : {"call fast float", "tail call float"}) {
    const std::string text = Module(
        absl::StrCat("  %v = ", call, " @llvm.log.f32(float 2.0)\n"
                     "  store float %v, ptr addrspace(1) %out, align 4"),
        "declare float @llvm.log.f32(float)");
    EXPECT_THAT(NormalizeMusaLlvmTextForLlvm14(text, "unsupported_log_call"),
                StatusIs(absl::StatusCode::kInvalidArgument,
                         HasSubstr("capability=log-profile")));
  }
  EXPECT_THAT(NormalizeMusaLlvmTextForLlvm14(
                  Module("  %v = call double @llvm.log.f64(double 2.0)",
...
TEST(MusaLlvm14CompatibilityTest,
     InlinesScalarBfloatLoadReturnAsAnExactIntegerStorageCopy) {
  const std::string helper =
      "define internal bfloat @gather_bfloat(ptr addrspace(1) %input, "
      "i32 %index) {\n"
      "entry:\n"
      "  %element = getelementptr inbounds [4 x bfloat], "
      "ptr addrspace(1) %input, i32 0, i32 %index\n"
      "  %loaded = load bfloat, ptr addrspace(1) %element, align 2\n"
      "  ret bfloat %loaded\n"
      "}\n";
  const std::string text = Module(
      "  %value = call bfloat @gather_bfloat(ptr addrspace(1) %input, "
      "i32 %index)\n"
      "  store bfloat %value, ptr addrspace(1) %out, align 2",
      helper, "", "", "ptr addrspace(1) %out, ptr addrspace(1) %input, "
                      "i32 %index");

  absl::StatusOr<MusaLlvm14CompatibilityResult> normalized =
      NormalizeMusaLlvmTextForLlvm14(text, "bfloat_load_return_helper");
  ASSERT_THAT(normalized, IsOk());
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("@gather_bfloat")));
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("load i16"));
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("store i16"));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("load bfloat")));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("store bfloat")));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("ret bfloat")));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("fptrunc")));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("fpext")));
  EXPECT_THAT(
      ValidateMusaBridgeIr(normalized->normalized_llvm, normalized->metadata),
      IsOk());
}
...
TEST(MusaLlvm14CompatibilityTest,
     RejectsUnqualifiedLoadsAndSelectedValuesAsBfloatHelperReturns) {
  for (const char* body : {
           "  %loaded = load volatile bfloat, ptr addrspace(1) %input, align 2\n"
           "  ret bfloat %loaded\n",
           "  %loaded = load bfloat, ptr addrspace(1) %input, align 4\n"
           "  ret bfloat %loaded\n",
           "  %loaded = load bfloat, ptr addrspace(1) %input, align 2\n"
           "  %selected = select i1 %condition, bfloat %loaded, "
           "bfloat 0.000000e+00\n"
           "  ret bfloat %selected\n",
       }) {
    const std::string helper = absl::StrCat(
        "define internal bfloat @unqualified_load(ptr addrspace(1) %input, "
        "i1 %condition) {\nentry:\n",
        body, "}\n");
    const std::string text = Module(
        "  %value = call bfloat @unqualified_load(ptr addrspace(1) %out, "
        "i1 true)\n"
        "  store bfloat %value, ptr addrspace(1) %out, align 2",
        helper);
    EXPECT_THAT(
        NormalizeMusaLlvmTextForLlvm14(text, "bfloat_unqualified_load_return"),
        StatusIs(absl::StatusCode::kInvalidArgument,
...
TEST(MusaLlvm14CompatibilityTest,
     InlinesTwoWayRoundedBfloatHelperReturnWithoutRerounding) {
  const std::string helper =
      "define internal bfloat @rope_half(i1 %condition, float %lhs, "
      "float %rhs) {\nentry:\n"
      "  br i1 %condition, label %left, label %right\n"
      "left:\n  %a = fptrunc float %lhs to bfloat\n"
      "  br label %merge\n"
      "right:\n  %b = fptrunc float %rhs to bfloat\n"
      "  br label %merge\n"
      "merge:\n  %joined = phi bfloat [ %a, %left ], [ %b, %right ]\n"
      "  ret bfloat %joined\n}\n";
  const std::string text = Module(
      "  %value = call bfloat @rope_half(i1 %condition, float %lhs, "
      "float %rhs)\n"
      "  store bfloat %value, ptr addrspace(1) %out, align 2",
      helper, "", "",
      "ptr addrspace(1) %out, i1 %condition, float %lhs, float %rhs");
  auto normalized = NormalizeMusaLlvmTextForLlvm14(text, "rope_half_join");
  ASSERT_THAT(normalized, IsOk());
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("@rope_half")));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("bfloat")));
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("phi i16"));
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("store i16"));
  EXPECT_THAT(
      ValidateMusaBridgeIr(normalized->normalized_llvm, normalized->metadata),
      IsOk());
...
TEST(MusaLlvm14CompatibilityTest,
     InlinesNestedLocalConversionHelpersBeforeTheirRoundedCaller) {
  const std::string helper =
      "define internal bfloat @narrow(float %x) {\n"
      "  %v = fptrunc float %x to bfloat\n  ret bfloat %v\n}\n"
      "define internal float @wide(float %x) {\n"
      "  %v = call bfloat @narrow(float %x)\n"
      "  %f = fpext bfloat %v to float\n  ret float %f\n}\n"
      "define internal bfloat @joined(i1 %cond, float %x, float %y) {\n"
      "  br i1 %cond, label %left, label %right\n"
      "left:\n  %a = call float @wide(float %x)\n"
      "  %an = fptrunc float %a to bfloat\n  br label %merge\n"
      "right:\n  %b = call float @wide(float %y)\n"
      "  %bn = fptrunc float %b to bfloat\n  br label %merge\n"
      "merge:\n  %v = phi bfloat [ %an, %left ], [ %bn, %right ]\n"
      "  ret bfloat %v\n}\n";
  const std::string text = Module(
      "  %v = call bfloat @joined(i1 %cond, float %x, float %y)\n"
      "  store bfloat %v, ptr addrspace(1) %out, align 2",
      helper, "", "", "ptr addrspace(1) %out, i1 %cond, float %x, float %y");
  auto normalized = NormalizeMusaLlvmTextForLlvm14(text, "nested_rope_helpers");
  ASSERT_THAT(normalized, IsOk());
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("@narrow")));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("@wide")));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("@joined")));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("bfloat")));
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("phi i16"));
  EXPECT_THAT(
      ValidateMusaBridgeIr(normalized->normalized_llvm, normalized->metadata),
...
TEST(MusaLlvm14CompatibilityTest,
     RejectsRecursiveWritingAndExternalScalarHelperDependencies) {
  for (const char* body : {
           "  %v = call float @dependency(float %x)\n  ret float %v\n",
           "  store float %x, ptr null, align 4\n  ret float %x\n",
           "  %v = call float @external(float %x)\n  ret float %v\n",
       }) {
    const std::string helper =
        absl::StrCat(std::string(body).find("@external") != std::string::npos
                         ? "declare float @external(float)\n"
                         : "",
                     "define internal float @dependency(float %x) {\n", body,
                     "}\n"
                     "define internal bfloat @narrow(float %x) {\n"
                     "  %v = call float @dependency(float %x)\n"
                     "  %n = fptrunc float %v to bfloat\n  ret bfloat %n\n}\n");
    const std::string text = Module(
        "  %v = call bfloat @narrow(float 1.000000e+00)\n"
        "  store bfloat %v, ptr addrspace(1) %out, align 2",
        helper);
    EXPECT_THAT(NormalizeMusaLlvmTextForLlvm14(text, "invalid_dependency"),
                StatusIs(absl::StatusCode::kInvalidArgument));
  }
}

TEST(MusaLlvm14CompatibilityTest, BoundsLocalHelperDependencyDepth) {
  std::string helper;
  for (int i = 0; i < 10; ++i) {
    absl::StrAppend(&helper, "define internal float @chain", i,
                    "(float %x) {\n");
    if (i == 9) {
      absl::StrAppend(&helper, "  ret float %x\n}\n");
    } else {
      absl::StrAppend(&helper, "  %v = call float @chain", i + 1,
                      "(float %x)\n  ret float %v\n}\n");
    }
  }
  absl::StrAppend(&helper,
                  "define internal bfloat @root(float %x) {\n"
                  "  %v = call float @chain0(float %x)\n"
                  "  %n = fptrunc float %v to bfloat\n  ret bfloat %n\n}\n");
  const std::string text = Module(
      "  %v = call bfloat @root(float 1.000000e+00)\n"
      "  store bfloat %v, ptr addrspace(1) %out, align 2",
      helper);
  EXPECT_THAT(NormalizeMusaLlvmTextForLlvm14(text, "deep_dependencies"),
              StatusIs(absl::StatusCode::kInvalidArgument,
                       HasSubstr("local helper dependency bound exceeded")));
}

TEST(MusaLlvm14CompatibilityTest,
     PreservesLoadedBfloatBitsThroughNestedAttentionTileJoins) {
  const std::string text = Module(
      "  br i1 %outer, label %left, label %right\n"
      "left:\n  br i1 %inner, label %a, label %b\n"
      "a:\n  %av = load bfloat, ptr addrspace(1) %input, align 2\n"
      "  br label %pair\n"
      "b:\n  %bp = getelementptr [3 x bfloat], ptr addrspace(1) %input, "
      "i32 0, i32 1\n"
      "  %bv = load bfloat, ptr addrspace(1) %bp, align 2\n"
      "  br label %pair\n"
      "pair:\n  %pv = phi bfloat [ %av, %a ], [ %bv, %b ]\n"
      "  br label %merge\n"
      "right:\n  %cp = getelementptr [3 x bfloat], ptr addrspace(1) %input, "
      "i32 0, i32 2\n"
      "  %cv = load bfloat, ptr addrspace(1) %cp, align 2\n"
      "  br label %merge\n"
      "merge:\n  %v = phi bfloat [ %pv, %pair ], [ %cv, %right ]\n"
      "  store bfloat %v, ptr addrspace(1) %out, align 2",
      "", "", "",
      "ptr addrspace(1) %out, ptr addrspace(1) %input, i1 %outer, i1 %inner");
  auto normalized = NormalizeMusaLlvmTextForLlvm14(text, "attention_tile_join");
  ASSERT_THAT(normalized, IsOk());
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("phi i16"));
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("load i16"));
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("store i16"));
...
TEST(MusaLlvm14CompatibilityTest, RejectsCyclicBfloatTransportJoins) {
  const std::string text = Module(
      "  %initial = load bfloat, ptr addrspace(1) %out, align 2\n"
      "  br label %loop\n"
      "loop:\n  %v = phi bfloat [ %initial, %entry ], [ %v, %again ]\n"
      "  br i1 %condition, label %again, label %exit\n"
      "again:\n  br label %loop\n"
      "exit:\n  store bfloat %v, ptr addrspace(1) %out, align 2",
      "", "", "", "ptr addrspace(1) %out, i1 %condition");
  EXPECT_THAT(
      NormalizeMusaLlvmTextForLlvm14(text, "cyclic_transport"),
      StatusIs(absl::StatusCode::kInvalidArgument, HasSubstr("bounded tree")));
}

TEST(MusaLlvm14CompatibilityTest,
     InlinesRoundedHelperResultsBeforeLoweringBfloatTransportJoins) {
  // Partial rotary transforms join a helper's already rounded result with a
  // negated/rounded branch. The join can also be returned by a local helper.
  const std::string narrow =
      "define internal bfloat @narrow(float %x) {\n"
      "  %v = fptrunc float %x to bfloat\n  ret bfloat %v\n}\n";
  const std::string join =
      "  br i1 %condition, label %left, label %right\n"
      "left:\n  %a = call bfloat @narrow(float %x)\n"
      "  br label %merge\n"
      "right:\n  %b = call bfloat @narrow(float %x)\n"
      "  %wide = fpext bfloat %b to float\n"
      "  %neg = fneg float %wide\n"
      "  %rounded = fptrunc float %neg to bfloat\n"
      "  br label %merge\n"
      "merge:\n  %v = phi bfloat [ %a, %left ], [ %rounded, %right ]\n";
  for (bool helper_join : {false, true}) {
    const std::string helper = helper_join
        ? absl::StrCat(narrow,
                      "define internal bfloat @joined(i1 %condition, float %x) {\n",
                      join, "  ret bfloat %v\n}\n")
        : narrow;
    const std::string body = helper_join
        ? "  %v = call bfloat @joined(i1 %condition, float %x)\n"
        : join;
    const std::string text = Module(
        absl::StrCat(body,
                    "  store bfloat %v, ptr addrspace(1) %out, align 2"),
        helper, "", "", "ptr addrspace(1) %out, i1 %condition, float %x");
    auto normalized = NormalizeMusaLlvmTextForLlvm14(text, "helper_transport");
    ASSERT_THAT(normalized, IsOk());
    EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("@narrow")));
    EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("@joined")));
    EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("bfloat")));
    EXPECT_THAT(normalized->normalized_llvm, HasSubstr("phi i16"));
    EXPECT_THAT(normalized->normalized_llvm, HasSubstr("fneg float"));
    EXPECT_THAT(normalized->normalized_llvm, HasSubstr("store i16"));
    EXPECT_THAT(
        ValidateMusaBridgeIr(normalized->normalized_llvm, normalized->metadata),
        IsOk());
...
TEST(MusaLlvm14CompatibilityTest, LowersScalarBfloatBitcastsWithoutRounding) {
  const std::string text = Module(
      "  %bits = load i16, ptr addrspace(1) %out, align 2\n"
      "  %narrow = bitcast i16 %bits to bfloat\n"
      "  %roundtrip = bitcast bfloat %narrow to i16\n"
      "  store i16 %roundtrip, ptr addrspace(1) %out, align 2\n"
      "  %value = load bfloat, ptr addrspace(1) %out, align 2\n"
      "  %value_bits = bitcast bfloat %value to i16\n"
      "  store i16 %value_bits, ptr addrspace(1) %out, align 2");
  absl::StatusOr<MusaLlvm14CompatibilityResult> normalized =
      NormalizeMusaLlvmTextForLlvm14(text, "scalar_bfloat_bitcast");
  ASSERT_THAT(normalized, IsOk());
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("to bfloat")));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("bitcast bfloat")));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("load bfloat")));
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("store i16 %bits"));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("fptrunc")));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("fpext")));
  EXPECT_THAT(
      ValidateMusaBridgeIr(normalized->normalized_llvm, normalized->metadata),
...
TEST(MusaLlvm14CompatibilityTest, RejectsUnqualifiedVectorBfloatBitcast) {
  const std::string text = Module(
      "  %value = load <2 x bfloat>, ptr addrspace(1) %out, align 2\n"
      "  %bits = bitcast <2 x bfloat> %value to i32\n"
      "  store i32 %bits, ptr addrspace(1) %out, align 4");
  EXPECT_THAT(NormalizeMusaLlvmTextForLlvm14(text, "vector_bfloat_bitcast"),
              StatusIs(absl::StatusCode::kInvalidArgument,
                       HasSubstr("only scalar bfloat/i16 bitcasts")));
}
...
TEST(MusaLlvm14CompatibilityTest, PreservesBfloatSharedStorageLayoutAndBarrier) {
  const std::string text = Module(
      "  %address = getelementptr [64 x bfloat], ptr addrspace(3) @tile, "
      "i32 0, i32 7\n"
      "  %value = load bfloat, ptr addrspace(1) %out, align 2\n"
      "  store bfloat %value, ptr addrspace(3) %address, align 2\n"
      "  call void @__xla_musa_v1_workgroup_barrier()\n"
      "  %shared = load bfloat, ptr addrspace(3) %address, align 2\n"
      "  store bfloat %shared, ptr addrspace(1) %out, align 2",
      "declare void @__xla_musa_v1_workgroup_barrier() convergent nounwind\n",
      "@tile = private addrspace(3) global [64 x bfloat] undef, align 16\n");
  auto normalized = NormalizeMusaLlvmTextForLlvm14(text, "shared_storage");
  ASSERT_THAT(normalized, IsOk());
  EXPECT_THAT(normalized->normalized_llvm,
              HasSubstr("@tile = private addrspace(3) global [64 x i16] undef, "
                        "align 16"));
  EXPECT_THAT(normalized->normalized_llvm,
              HasSubstr("call void @__xla_musa_v1_workgroup_barrier()"));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("bfloat")));
  EXPECT_THAT(
      ValidateMusaBridgeIr(normalized->normalized_llvm, normalized->metadata),
      IsOk());
...
TEST(MusaLlvm14CompatibilityTest, RejectsInitializedBfloatSharedStorage) {
  const std::string text = Module(
      "  store i16 0, ptr addrspace(1) %out, align 2", "",
      "@tile = private addrspace(3) global [64 x bfloat] zeroinitializer\n");
  EXPECT_THAT(NormalizeMusaLlvmTextForLlvm14(text, "initialized_shared"),
              StatusIs(absl::StatusCode::kInvalidArgument,
                       HasSubstr("bfloat-shared-storage")));
}

TEST(MusaLlvm14CompatibilityTest, LowersBfloatPhiContainingSelection) {
  const std::string text = Module(
      "  br i1 %outer, label %left, label %right\n"
      "left:\n"
      "  %a = load bfloat, ptr addrspace(1) %out, align 2\n"
      "  %b = fptrunc float %source to bfloat\n"
      "  %selected = select i1 %inner, bfloat %a, bfloat %b\n"
      "  br label %merge\n"
      "right:\n"
      "  %c = load bfloat, ptr addrspace(1) %out, align 2\n"
      "  br label %merge\n"
      "merge:\n"
      "  %joined = phi bfloat [ %selected, %left ], [ %c, %right ]\n"
      "  store bfloat %joined, ptr addrspace(1) %out, align 2",
      "", "", "", "ptr addrspace(1) %out, i1 %outer, i1 %inner, float %source");
  auto normalized = NormalizeMusaLlvmTextForLlvm14(text, "selection_join");
  ASSERT_THAT(normalized, IsOk());
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("select i1 %inner, i16"));
  EXPECT_THAT(normalized->normalized_llvm, HasSubstr("phi i16"));
  EXPECT_THAT(normalized->normalized_llvm, Not(HasSubstr("bfloat")));
  EXPECT_THAT(
      ValidateMusaBridgeIr(normalized->normalized_llvm, normalized->metadata),
...
TEST(MusaLlvm14CompatibilityTest,
     PadsSmallMp22ExportedGlobalsBeforeCollectingExactStorageSizes) {
  for (int64_t size : {64, 128, 256}) {
    for (bool constant : {false, true}) {
      const int address_space = constant ? 2 : 1;
      const std::string declaration =
          absl::StrCat("@aligned_global = protected addrspace(", address_space,
                       ") ", constant ? "constant" : "global", " [", size,
                       " x i8] zeroinitializer, align 256\n");
      const std::string body = absl::StrCat(
          "  %first = load volatile i8, ptr addrspace(", address_space,
          ") @aligned_global, align 256\n", "  %last = getelementptr [", size,
          " x i8], ptr addrspace(", address_space,
          ") @aligned_global, i64 0, i64 ", size - 1, "\n",
          "  %byte = load volatile i8, ptr addrspace(", address_space,
          ") %last, align 1\n",
          "  store i8 %byte, ptr addrspace(1) %out, align 1");
      absl::StatusOr<MusaLlvm14CompatibilityResult> normalized =
          NormalizeMusaLlvmTextForLlvm14(Module(body, "", declaration),
                                         "aligned_globals", "mp_22");
      ASSERT_THAT(normalized, IsOk());
      ASSERT_EQ(normalized->metadata.exported_globals.size(), 1);
      const MusaExportedGlobal& global =
          normalized->metadata.exported_globals[0];
      EXPECT_EQ(global.name, "aligned_global");
      EXPECT_EQ(global.address_space, address_space);
      EXPECT_EQ(global.alignment, 256);
      EXPECT_EQ(global.size, 256);
      EXPECT_EQ(global.kind, constant ? MusaExportedGlobalKind::kConstant
                                      : MusaExportedGlobalKind::kMutable);
      EXPECT_THAT(normalized->normalized_llvm,
                  HasSubstr(absl::StrCat("getelementptr [", size,
                                         " x i8], ptr addrspace(",
                                         address_space, ") @aligned_global")));
      EXPECT_THAT(normalized->normalized_llvm,
                  HasSubstr(absl::StrCat("load volatile i8, ptr addrspace(",
                                         address_space, ") @aligned_global")));
      EXPECT_EQ(absl::StrContains(normalized->normalized_llvm, "constant { ["),
                constant && size < 256);
    }
  }
}
...
TEST(MusaLlvm14CompatibilityTest,
     PreservesLeadingLiteralAndLeavesInputAndQualifiedSmallAlignmentUnchanged) {
  const std::string declaration =
      "@constant_data = constant [4 x i32] [i32 16, i32 -7, i32 42, i32 99], "
      "align 256\n";
  llvm::LLVMContext context;
  llvm::SMDiagnostic diagnostic;
  std::unique_ptr<llvm::Module> input = llvm::parseAssemblyString(
      Module("  store i32 1, ptr addrspace(1) %out, align 4", "", declaration),
      diagnostic, context);
  ASSERT_NE(input, nullptr);
  absl::StatusOr<MusaLlvm14CompatibilityResult> normalized =
      NormalizeMusaLlvmForLlvm14(*input, "literal_padding", "mp_22");
  ASSERT_THAT(normalized, IsOk());
  EXPECT_THAT(
      normalized->normalized_llvm,
      HasSubstr("{ [4 x i32], [240 x i8] } { [4 x i32] [i32 16, i32 -7, i32 "
                "42, i32 99], [240 x i8] zeroinitializer }, align 256"));
  EXPECT_EQ(input->getDataLayout()
                .getTypeAllocSize(
                    input->getGlobalVariable("constant_data")->getValueType())
                .getFixedValue(),
            16);
  EXPECT_EQ(normalized->metadata.exported_globals[0].size, 256);

  const std::string small_alignment =
      absl::StrReplaceAll(declaration, {{"align 256", "align 16"}});
  absl::StatusOr<MusaLlvm14CompatibilityResult> unchanged =
      NormalizeMusaLlvmTextForLlvm14(
          Module("  store i32 1, ptr addrspace(1) %out, align 4", "",
                 small_alignment),
          "small_alignment", "mp_22");
  ASSERT_THAT(unchanged, IsOk());
  EXPECT_EQ(unchanged->metadata.exported_globals[0].size, 16);
}
