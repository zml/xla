...
absl::Status ValidateCurrentTarget(const llvm::Module& module,
  ...
  if (module.getDataLayoutStr() != kMusaDataLayout) {
    return Rejected(module_name, "data-layout",
                    "module does not use the qualified MUSA data layout");
  }
...
llvm::Value* LowerFloatingMinMaxCall(llvm::CallInst& call) {
  ...
  if (propagates_nan) {
    ...
    llvm::Value* nan =
        new llvm::BitCastInst(nan_bits, type, prefix + ".canonical_nan",
                              call.getIterator());
...
absl::Status NormalizeScalarLogInputs(llvm::Module& module,
                                      absl::string_view module_name) {
  std::vector<llvm::CallInst*> calls;
  for (llvm::Function& function : module.functions()) {
    if (!function.isIntrinsic() ||
        function.getIntrinsicID() != llvm::Intrinsic::log) continue;
    if (!function.getReturnType()->isFloatTy()) {
      return Rejected(module_name, "log-profile",
                      "only scalar f32 logarithms are qualified");
    }
    for (llvm::User* user : function.users()) {
      auto* call = llvm::dyn_cast<llvm::CallInst>(user);
      if (call == nullptr || call->getCalledFunction() != &function ||
          call->getCallingConv() != llvm::CallingConv::C || call->isTailCall() ||
          call->hasOperandBundles() || !call->getAttributes().isEmpty() ||
          call->getFastMathFlags().any() || call->hasMetadata()) {
        return Rejected(module_name, "log-profile",
                        "logarithms require plain scalar calls");
      }
      calls.push_back(call);
    }
  }
  for (llvm::CallInst* call : calls) {
    llvm::IRBuilder<> builder(call);
    llvm::Value* input = call->getArgOperand(0);
    llvm::Value* bits = builder.CreateBitCast(input, builder.getInt32Ty());
    llvm::Value* magnitude = builder.CreateAnd(bits, 0x7fffffff);
    llvm::Value* subnormal = builder.CreateAnd(
        builder.CreateICmpUGT(magnitude, builder.getInt32(0)),
        builder.CreateICmpULT(magnitude, builder.getInt32(0x00800000)),
        "log_is_subnormal");
    // Vendor log flushes subnormal inputs. Interpret the subnormal fraction
    // as an exact normal integer: x = fraction * 2^-149. Integer inspection
    // and conversion avoid any floating operation on the subnormal itself.
    llvm::Value* fraction = builder.CreateAnd(magnitude, 0x007fffff);
    llvm::Value* normal = builder.CreateUIToFP(fraction, builder.getFloatTy());
    llvm::Value* negative = builder.CreateICmpSLT(bits, builder.getInt32(0));
    normal = builder.CreateSelect(negative, builder.CreateFNeg(normal), normal);
    llvm::Value* argument = builder.CreateSelect(subnormal, normal, input);
    llvm::Value* logarithm = builder.CreateCall(call->getCalledFunction(),
                                               {argument}, "log_normal_input");
    llvm::Value* corrected = builder.CreateFSub(
        logarithm, llvm::ConstantFP::get(builder.getFloatTy(),
                                        103.27892990343185),
        "log_subnormal_result");  // 149 * ln(2).
    call->replaceAllUsesWith(builder.CreateSelect(subnormal, corrected, logarithm));
    call->eraseFromParent();
  }
  return absl::OkStatus();
}
...
bool IsQualifiedBfloatHelperReturn(const llvm::Value* value,
                                   unsigned depth = 0) {
  if (depth > 8) return false;
  // Validate before mechanical inlining, but follow a local call's return
  // value when it feeds a transport join. Partial rotary transforms emit
  // exactly this shape. The normal helper validator still checks signatures,
  // call attributes, dependencies and expansion bounds before any inlining.
  // Following returns shares the join depth bound, so recursion cannot pass.
  if (const auto* call = llvm::dyn_cast_or_null<llvm::CallInst>(value)) {
    const llvm::Function* callee = call->getCalledFunction();
    if (!call->getType()->isBFloatTy() || callee == nullptr ||
        callee->isDeclaration() || !callee->hasLocalLinkage()) {
      return false;
    }
    ...
    for (const llvm::BasicBlock& block : *callee) {
      ...
      if (++return_sites != 1 ||
          !IsQualifiedBfloatHelperReturn(ret->getReturnValue(), depth + 1)) {
        return false;
      }
    }
    return return_sites == 1;
  }
  if (const auto* truncation =
          llvm::dyn_cast_or_null<llvm::FPTruncInst>(value)) {
    return truncation->getOperand(0)->getType()->isFloatTy();
  }
  if (const auto* select = llvm::dyn_cast_or_null<llvm::SelectInst>(value)) {
    return select->getType()->isBFloatTy() &&
           select->getCondition()->getType()->isIntegerTy(1) &&
           IsQualifiedBfloatHelperReturn(select->getTrueValue(), depth + 1) &&
           IsQualifiedBfloatHelperReturn(select->getFalseValue(), depth + 1);
  }
  // Selection and concatenation use bounded trees of two-way joins.
  // Leaves are already rounded values or exact scalar storage loads. The
  // integer PHI lowerer preserves their bits; cycles exceed the depth bound.
  if (const auto* phi = llvm::dyn_cast_or_null<llvm::PHINode>(value)) {
    if (!phi->getType()->isBFloatTy() || phi->getNumIncomingValues() != 2 ||
        phi->getIncomingBlock(0) == phi->getIncomingBlock(1)) {
      return false;
    }
    for (const llvm::Value* incoming : phi->incoming_values()) {
      if (!IsQualifiedBfloatHelperReturn(incoming, depth + 1)) {
        return false;
      }
    }
    return true;
  }
  // A scalar load forwarded by an internal gather helper is already within
  // the memory profile. Mechanical inlining exposes the same load to the
  // integer lowerer, preserving all 16 storage bits without another rounding.
  const auto* load = llvm::dyn_cast_or_null<llvm::LoadInst>(value);
  return load != nullptr && load->getType()->isBFloatTy() && load->isSimple() &&
         load->getAlign().value() == 2;
}
    ...
    if (TypeContainsBfloat(function.getFunctionType())) {
      ...
      if (function.getReturnType()->isBFloatTy()) {
        ...
        for (const llvm::BasicBlock& block : function) {
          ...
          if (ret == nullptr) continue;
          ++return_sites;
          if (!IsQualifiedBfloatHelperReturn(ret->getReturnValue())) {
            return Rejected(
                module_name, "bfloat-integer-profile",
                "local helper bfloat return requires a bounded tree of "
                "scalar loads or f32 truncations");
          }
        }
        if (return_sites != 1) {
          return Rejected(
    ...
    for (const llvm::BasicBlock& block : function) {
      for (const llvm::Instruction& instruction : block) {
        ...
        if (const auto* bitcast =
          ...
          llvm::Type* source = bitcast->getOperand(0)->getType();
          llvm::Type* target = bitcast->getType();
          if (!((source->isBFloatTy() && target->isIntegerTy(16)) ||
                (source->isIntegerTy(16) && target->isBFloatTy()))) {
            return Rejected(module_name, "bfloat-integer-profile",
                            "only scalar bfloat/i16 bitcasts are qualified");
          }
          continue;
        }
        ...
        if (const auto* phi = llvm::dyn_cast<llvm::PHINode>(&instruction)) {
          if (!IsQualifiedBfloatHelperReturn(phi)) {
            return Rejected(module_name, "bfloat-integer-profile",
                            "bfloat phi requires a bounded tree of scalar "
                            "loads or f32 truncations");
          }
          continue;
        }
        ...
        if (const auto* ret = llvm::dyn_cast<llvm::ReturnInst>(&instruction)) {
          if (!function.getReturnType()->isBFloatTy() ||
              !IsQualifiedBfloatHelperReturn(ret->getReturnValue())) {
            return Rejected(module_name, "bfloat-integer-profile",
                            "bfloat return requires a bounded tree of "
                            "scalar loads or f32 truncations");
          }
          continue;
        }
...
absl::Status InlineQualifiedBfloatHelpers(llvm::Module& module,
  ...
  llvm::DenseMap<llvm::Function*, unsigned> state;
  size_t helper_instructions = 0;
  std::function<absl::Status(llvm::Function*, unsigned)> visit;
  visit = [&](llvm::Function* candidate, unsigned depth) -> absl::Status {
    if (depth > 8 || (state.lookup(candidate) == 0 && state.size() >= 64)) {
      return Rejected(module_name, "bfloat-helper-inline",
                      "local helper dependency bound exceeded");
    }
    if (state.lookup(candidate) == 1) {
      return Rejected(module_name, "bfloat-helper-inline",
                      "recursive local helpers are not qualified");
    }
    if (state.lookup(candidate) == 2) return absl::OkStatus();
    state[candidate] = 1;
    llvm::Function& function = *candidate;
    if (function.isDeclaration() || !function.hasLocalLinkage() ||
        function.getCallingConv() != llvm::CallingConv::C ||
    ...
    for (const llvm::BasicBlock& block : function) {
      helper_instructions += block.size();
      if (helper_instructions > 8192) {
        return Rejected(module_name, "bfloat-helper-inline",
                        "local helper instruction bound exceeded");
      }
      for (const llvm::Instruction& instruction : block) {
        ...
        if (call == nullptr) {
          // Newly admitted F32 dependencies may read inputs but cannot hide
          // writes or synchronization inside a scalar conversion helper.
          if (!TypeContainsBfloat(function.getFunctionType()) &&
              (instruction.mayWriteToMemory() ||
               llvm::isa<llvm::FenceInst>(instruction) ||
               (llvm::isa<llvm::LoadInst>(instruction) &&
                !llvm::cast<llvm::LoadInst>(instruction).isSimple()))) {
            return Rejected(module_name, "bfloat-helper-inline",
                            "scalar helper dependencies must be read-only");
          }
          continue;
        }
        ...
        if (plain_call == nullptr || callee == nullptr ||
          ...
          return Rejected(module_name, "bfloat-helper-inline",
                          "helper dependencies require plain direct calls");
        }
        if (callee->isIntrinsic()) {
          if (!IsReviewedPureIntrinsic(callee->getIntrinsicID())) {
            return Rejected(module_name, "bfloat-helper-inline",
                            "helper intrinsic is not reviewed");
          }
        } else {
          if (!TypeContainsBfloat(callee->getFunctionType()) &&
              !callee->getReturnType()->isFloatTy()) {
            return Rejected(module_name, "bfloat-helper-inline",
                            "helper dependency must return scalar f32");
          }
          if (absl::Status status = visit(callee, depth + 1); !status.ok()) {
            return status;
          }
        }
      }
  ...
  for (llvm::Function& function : module.functions()) {
    ...
    if (absl::Status status = visit(&function, 0); !status.ok()) return status;
  }

  // Dependencies precede their callers. Collect users anew after each helper
  // is inlined so no saved instruction pointer is invalidated by an ancestor.
  size_t expanded_instructions = 0;
  for (llvm::Function* helper : helpers) {
    std::vector<llvm::CallInst*> calls;
    size_t size = 0;
    for (const llvm::BasicBlock& block : *helper) size += block.size();
    for (llvm::User* user : helper->users()) {
      calls.push_back(llvm::cast<llvm::CallInst>(user));
    }
    for (llvm::CallInst* call : calls) {
      expanded_instructions += size;
      if (expanded_instructions > 65536) {
        return Rejected(module_name, "bfloat-helper-inline",
                        "expanded helper instruction bound exceeded");
      }
...
absl::Status LowerBfloatStorageGeps(llvm::Module& module,
  ...
  for (llvm::GetElementPtrInst* gep : geps) {
    ...
    llvm::GetElementPtrInst* encoded = llvm::GetElementPtrInst::Create(
        ...
        BfloatBitsName(*gep), gep->getIterator());
...
class BfloatIntegerLowerer {
 public:
  ...
  absl::Status Lower() {
    ...
    std::vector<llvm::BitCastInst*> bitcasts_to_integer;
    ...
    for (llvm::Function& function : module_.functions()) {
      for (llvm::BasicBlock& block : function) {
        for (llvm::Instruction& instruction : block) {
          ...
          if (auto* bitcast = llvm::dyn_cast<llvm::BitCastInst>(&instruction);
              bitcast != nullptr &&
              bitcast->getOperand(0)->getType()->isBFloatTy()) {
            bitcasts_to_integer.push_back(bitcast);
          }
    ...
    for (llvm::BitCastInst* bitcast : bitcasts_to_integer) {
      if (HasUnsupportedInstructionMetadata(*bitcast)) {
        return Rejected(module_name_, "bfloat-integer-lowering",
                        "bfloat bitcast carries unsupported metadata");
      }
      absl::StatusOr<llvm::Value*> encoded = Encode(bitcast->getOperand(0));
      if (!encoded.ok()) return encoded.status();
      bitcast->replaceAllUsesWith(*encoded);
    }
  ...
  absl::StatusOr<llvm::Value*> Encode(llvm::Value* value) {
    ...
    if (auto* phi = llvm::dyn_cast<llvm::PHINode>(instruction)) {
      auto* encoded = llvm::PHINode::Create(
          encoded_type, phi->getNumIncomingValues(), BfloatBitsName(*phi),
          phi->getIterator());
      encoded_[value] = encoded;
    ...
    if (auto* load = llvm::dyn_cast<llvm::LoadInst>(instruction)) {
      ...
      encoded = lowered;
    } else if (auto* bitcast = llvm::dyn_cast<llvm::BitCastInst>(instruction)) {
      encoded = bitcast->getOperand(0);
...
absl::Status LowerBfloatSharedStorage(llvm::Module& module,
                                    absl::string_view module_name) {
  std::vector<llvm::GlobalVariable*> globals;
  for (llvm::GlobalVariable& global : module.globals()) {
    if (!TypeContainsBfloat(global.getValueType())) continue;
    if (global.getAddressSpace() != 3 || !global.hasLocalLinkage() ||
        global.isConstant() || global.isThreadLocal() ||
        global.isExternallyInitialized() || global.hasMetadata() ||
        !global.hasInitializer() ||
        !llvm::isa<llvm::UndefValue>(global.getInitializer()) ||
        !IsQualifiedBfloatStorageType(global.getValueType())) {
      return Rejected(module_name, "bfloat-shared-storage",
                      "only local uninitialized flat BF16 shared arrays are "
                      "qualified");
    }
  ...
  for (llvm::GlobalVariable* global : globals) {
    auto* original = llvm::cast<llvm::ArrayType>(global->getValueType());
    auto* encoded = llvm::ArrayType::get(
        llvm::Type::getInt16Ty(module.getContext()), original->getNumElements());
    if (layout.getTypeAllocSize(original) != layout.getTypeAllocSize(encoded) ||
        layout.getABITypeAlign(original) != layout.getABITypeAlign(encoded)) {
      return Rejected(module_name, "bfloat-shared-storage",
                      "i16 shared storage changes the BF16 array layout");
    }
    auto* replacement = new llvm::GlobalVariable(
        module, encoded, false, global->getLinkage(),
        llvm::UndefValue::get(encoded), "", global,
        global->getThreadLocalMode(), global->getAddressSpace());
    replacement->copyAttributesFrom(global);
    replacement->takeName(global);
    // Opaque pointers retain the same address. Storage GEPs are lowered
    // separately; their offsets and all barriers remain unchanged.
...
absl::Status LowerBfloatMemoryOperations(llvm::Module& module,
  ...
  if (absl::Status status = LowerBfloatSharedStorage(module, module_name);
      !status.ok()) {
    return status;
  }
...
bool PadSmallExportedGlobalStorage(llvm::Module& module) {
  // SDK 5.1's mp_22 module loader allocates small data sections from a
  // 128-byte pool even when their ELF section and symbol alignment is 256.
  // Give each exported object with the qualified XLA alignment at least one
  // complete 256-byte storage unit. This changes the actual module-global
  // backing allocation, so both muModuleGetGlobal and direct LLVM references
  // see the same aligned storage. Logical bytes remain at offset zero.
  constexpr uint64_t kGlobalStorageAlignment = 256;
  const llvm::DataLayout& layout = module.getDataLayout();
  std::vector<llvm::GlobalVariable*> globals;
  for (llvm::GlobalVariable& global : module.globals()) {
    if (global.hasLocalLinkage() || global.isDeclaration()) continue;
    const uint64_t alignment =
        global.getAlign()
            ? global.getAlign()->value()
            : layout.getABITypeAlign(global.getValueType()).value();
    const llvm::TypeSize size = layout.getTypeAllocSize(global.getValueType());
    if (alignment == kGlobalStorageAlignment && !size.isScalable() &&
        size.getFixedValue() != 0 &&
        size.getFixedValue() < kGlobalStorageAlignment) {
      globals.push_back(&global);
    }
  }
  for (llvm::GlobalVariable* global : globals) {
    llvm::Type* value_type = global->getValueType();
    const uint64_t bytes = layout.getTypeAllocSize(value_type).getFixedValue();
    llvm::ArrayType* padding_type =
        llvm::ArrayType::get(llvm::Type::getInt8Ty(module.getContext()),
                             kGlobalStorageAlignment - bytes);
    llvm::StructType* storage_type =
        llvm::StructType::get(module.getContext(), {value_type, padding_type});
    llvm::Constant* initializer = llvm::ConstantStruct::get(
        storage_type, {global->getInitializer(),
                       llvm::ConstantAggregateZero::get(padding_type)});
    auto* padded = new llvm::GlobalVariable(
        module, storage_type, global->isConstant(), global->getLinkage(),
        initializer, /*Name=*/"", /*InsertBefore=*/global,
        global->getThreadLocalMode(), global->getAddressSpace(),
        global->isExternallyInitialized());
    padded->copyAttributesFrom(global);
    padded->takeName(global);
    // The interchange has opaque pointers. Existing GEP source types,
    // loads, stores and constant expressions continue to address the
    // original leading object without casts, offsets or name changes.
...
absl::StatusOr<MusaLlvm14CompatibilityResult> NormalizeMusaLlvmForLlvm14(
  ...
  if (!IsSupportedMusaTargetArchitecture(architecture)) {
    return Rejected(module_name, "architecture",
                    "target is not a qualified mp_21 or mp_22 architecture");
  }
  ...
  metadata.architecture = std::string(architecture);
  ...
  if (absl::Status status = NormalizeScalarLogInputs(*module, module_name);
      !status.ok()) {
    return status;
  }
  ...
  std::string current_text = PrintModule(*module);
  ...
  // Validate the original objects before replacement: unsupported metadata
  // or linker state must never disappear as a side effect of padding.
  if (architecture == kMusaS4000TargetArchitecture &&
      PadSmallExportedGlobalStorage(*module)) {
    metadata.exported_globals.clear();
    if (absl::Status status =
            CollectExportedGlobals(*module, module_name, metadata);
        !status.ok()) {
      return status;
    }
    current_text = PrintModule(*module);
    if (absl::Status status = ValidateMusaBridgeIr(current_text, metadata);
        !status.ok()) {
      return status;
    }
...
absl::StatusOr<MusaLlvm14CompatibilityResult> NormalizeMusaLlvmTextForLlvm14(
  ...
  return NormalizeMusaLlvmForLlvm14(*module, module_name, architecture);
}
