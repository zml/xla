...
TEST_F(MccBundleCodegenTest, CanonicalProviderContractFreezesBehavior) {
  ...
  EXPECT_THAT(
      contract,
      HasSubstr("targets=mtgpu-mt-musa/mp_21,mtgpu-mt-musa/mp_22\n"));
  EXPECT_THAT(contract, HasSubstr("mcc_argv=-mtgpu|--musa-device-only|"));
  EXPECT_THAT(contract, HasSubstr("--musa-path={toolkit_root}|"));
...
TEST_F(MccBundleCodegenTest, UsesS4000ArchitectureEndToEnd) {
  MccBundleCodegenOptions options = Options();
  options.architecture = "mp_22";
  absl::StatusOr<MccBundleCodegenResult> result =
      CompileVerifiedMusaLlvmWithMcc(VendorLlvm(), options);
  ASSERT_TRUE(result.ok()) << result.status();
  EXPECT_EQ(result->selected_bundle_id, "musa-mtgpu-mt-musa---mp_22");
  ExpectProviderDirectoryWasRemoved();
}
...
TEST_F(MccBundleCodegenTest, RejectsAnythingOutsideFrozenContract) {
  ...
  EXPECT_THAT(
      ...
      StatusIs(absl::StatusCode::kInvalidArgument,
               HasSubstr("qualified mtgpu-mt-musa")));
...
TEST_F(MccBundleCodegenTest, GuardedProviderRetainsO2AndScopesGuardToS4000) {
  EXPECT_EQ(MccBundleProviderName(true), "mcc-bundle-v2");
  EXPECT_THAT(MccBundleProviderCanonicalText(true),
              HasSubstr("skip-only:LoopLoadEliminationPass;architecture:mp_22"));
  MccBundleCodegenOptions options = Options();
  options.llvm_pass_plugin_path = HelperPath();
  options.architecture = "mp_22";
  auto s4000 = CompileVerifiedMusaLlvmWithMcc(VendorLlvm("guard"), options);
  ASSERT_TRUE(s4000.ok()) << s4000.status();
  options.architecture = "mp_21";
  auto s80 = CompileVerifiedMusaLlvmWithMcc(VendorLlvm(), options);
  ASSERT_TRUE(s80.ok()) << s80.status();
  ExpectProviderDirectoryWasRemoved();
}
