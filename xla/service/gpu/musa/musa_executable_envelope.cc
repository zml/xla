...
BuildMusaExecutableEnvelope(
  ...
  if (!stream_executor::musa::IsQualifiedMusaTargetContract(
          compute_capability.architecture(), compute_capability.major(),
          compute_capability.minor(), compute_capability.hardware_warp_size(),
    ...
    return InvalidEnvelope(
        "device capability does not match a qualified MUSA target");
  }
  ...
  const std::string expected_target = absl::StrCat(
  ...
  musa->set_architecture(compute_capability.architecture());
