...
TEST(MusaBoundedSubprocessTest, TimeoutKillsTheProcessGroup) {
  ...
  MusaSubprocessOptions options =
      Options({"--child-sentinel=" + sentinel, "--child-delay-ms=1000",
               "--sleep-ms=2000"});
  options.limits.timeout = std::chrono::milliseconds(500);
  const auto started = std::chrono::steady_clock::now();
  absl::StatusOr<MusaSubprocessResult> result =
      RunMusaBoundedSubprocess(options);
  const auto elapsed = std::chrono::steady_clock::now() - started;
  ...
  EXPECT_FALSE(result->exited_successfully());
  EXPECT_LT(elapsed, std::chrono::seconds(2));
  EXPECT_EQ(result->stdout_text, "child-ready");
...
TEST(MusaBoundedSubprocessTest,
  ...
  unlink(sentinel.c_str());
  MusaSubprocessOptions options =
      Options({"--child-sentinel=" + sentinel, "--child-delay-ms=2500",
               "--child-detach", "--sleep-ms=5000"});
  // Include descriptor cleanup and exec startup in the timeout, while keeping
  // the detached child alive beyond timeout plus the runner's pipe grace.
  options.limits.timeout = std::chrono::milliseconds(500);
  const auto started = std::chrono::steady_clock::now();
  absl::StatusOr<MusaSubprocessResult> result =
      RunMusaBoundedSubprocess(options);
  ...
  EXPECT_LT(elapsed, std::chrono::seconds(2));
  EXPECT_EQ(result->stdout_text, "child-ready");
  // The runner returns before the detached child's 2500 ms delay expires.
  // Observe its eventual escape separately from the runner's wall-time bound.
  const auto sentinel_deadline =
      std::chrono::steady_clock::now() + std::chrono::seconds(5);
  while (access(sentinel.c_str(), F_OK) != 0 &&
