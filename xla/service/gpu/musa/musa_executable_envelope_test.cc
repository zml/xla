...
stream_executor::DeviceDescription S4000Device() {
  stream_executor::DeviceDescription device = S80Device();
  device.set_gpu_compute_capability(stream_executor::GpuComputeCapability(
      stream_executor::MusaComputeCapability("mp_22", 2, 2,
                                             /*hardware_warp_size=*/128,
                                             /*logical_subgroup_size=*/32)));
  device.set_runtime_version(stream_executor::SemanticVersion(5, 1, 0));
  device.set_driver_version(stream_executor::SemanticVersion(5, 1, 0));
  device.set_kernel_mode_driver_version(
      stream_executor::SemanticVersion(5, 1, 0));
  device.set_compile_time_toolkit_version(
      stream_executor::SemanticVersion(5, 1, 0));
  return device;
}
...
TEST(MusaExecutableEnvelopeTest, BuildsS4000Identity) {
  MusaCompilationCapabilities capabilities;
  capabilities.target = "mtgpu-mt-musa/mp_22";
  ASSERT_OK_AND_ASSIGN(
      stream_executor::ExecutableAbiVersion envelope,
      BuildMusaExecutableEnvelope(S4000Device(), Identity(), capabilities,
                                  MusaCompilationOptions(),
                                  /*main_binary=*/{}));

  const auto& musa = envelope.proto().musa_platform_version();
  EXPECT_EQ(musa.architecture(), "mp_22");
  EXPECT_EQ(musa.required_toolkit_version(), "5.1.0");
  EXPECT_THAT(ValidateMusaExecutableEnvelope(envelope, {}), IsOk());
}

TEST(MusaExecutableEnvelopeTest, RejectsDeviceFactsOutsideQualifiedTargets) {
  ...
  for (const stream_executor::MusaComputeCapability& capability : unsupported) {
    ...
    EXPECT_THAT(BuildMusaExecutableEnvelope(
                ...
                StatusIs(absl::StatusCode::kInvalidArgument,
                         HasSubstr("qualified MUSA target")));
  }
