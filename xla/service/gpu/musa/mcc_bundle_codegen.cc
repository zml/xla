...
constexpr char kMccBundleProviderCanonicalText[] =
    ...
    "targets=mtgpu-mt-musa/mp_21,mtgpu-mt-musa/mp_22\n"
    ...
    "mcc_argv=-mtgpu|--musa-device-only|--musa-path={toolkit_root}|"
    "--offload-arch={architecture}|-O2|"
...
absl::Status ValidateOptions(absl::string_view verified_vendor_llvm,
  ...
  if (!IsSafeAbsolutePath(options.mcc_path) ||
      ...
      (!options.llvm_pass_plugin_path.empty() &&
       !IsSafeAbsolutePath(options.llvm_pass_plugin_path))) {
  ...
  if (options.target_triple != stream_executor::musa::kMusaTargetTriple ||
      !stream_executor::musa::IsQualifiedMusaArchitecture(
          options.architecture) ||
      options.optimization_level != 2 || !options.opaque_pointers) {
    return absl::InvalidArgumentError(
        "MCC bundle codegen supports only qualified mtgpu-mt-musa "
        "mp_21/mp_22 O2 opaque-pointer options");
  }
...
absl::StatusOr<std::string> SelectBundleId(
  ...
  if (matching_ids.size() != 1) {
    return absl::FailedPreconditionError(absl::StrCat(
        ...
        " matching mtgpu-mt-musa/", options.architecture,
        " images; exactly one is required"));
  }
...
absl::string_view MccBundleProviderName(bool use_pass_guard) {
  return use_pass_guard ? "mcc-bundle-v2" : kMccBundleProviderName;
}

absl::string_view MccBundleProviderCanonicalText(bool use_pass_guard) {
  if (use_pass_guard) {
    static const std::string contract = absl::StrCat(
        kMccBundleProviderCanonicalText,
        "extension=xla-musa-sdk51-loop-load-guard-v1\n"
        "pass_guard=skip-only:LoopLoadEliminationPass;architecture:mp_22\n"
        "pass_guard_argv=insert-before:-x;value:-fpass-plugin={llvm_pass_plugin}\n");
    return contract;
  }
...
absl::StatusOr<MccBundleCodegenResult> CompileVerifiedMusaLlvmWithMcc(
  ...
  const std::string toolkit_root = Dirname(Dirname(options.mcc_path));
  ...
  std::vector<std::string> mcc_arguments = {
      "-mtgpu", "--musa-device-only",
      absl::StrCat("--musa-path=", toolkit_root),
      absl::StrCat("--offload-arch=", options.architecture),
      "-O2", "-mllvm", "-opaque-pointers"};
  if (!options.llvm_pass_plugin_path.empty() && options.architecture == "mp_22") {
    mcc_arguments.push_back(
        absl::StrCat("-fpass-plugin=", options.llvm_pass_plugin_path));
  }
  mcc_arguments.insert(mcc_arguments.end(),
                       {"-x", "ir", input_bundle_path, "-c", "-o", output_bundle_path});
  result = RunStage("MCC codegen", options.mcc_path, std::move(mcc_arguments),
