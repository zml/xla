...
The fast kernels are `matmul_simdgroup_8x8` and
`matmul_relu_simdgroup_8x8`. They use Metal simdgroup matrix
multiply-accumulate operations and currently require `M % 16 == 0`,
`N % 32 == 0`, and `K % 8 == 0`. The ReLU variant fuses the activation into
the matmul epilogue before the simdgroup matrix store.
